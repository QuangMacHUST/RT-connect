...
**Ngày:** 2026-09-13
**Mốc yêu cầu:** UX1.3.
**Nguồn:** business-analysis.md v1.3; **kế hoạch:** plan.md v5.3; **danh mục bộ tính:** [docs/pylinac-qa-catalog.md](docs/pylinac-qa-catalog.md) v1.3.
...
- Với `TG51Photon`, `measured_pdd10` là bắt buộc ở biên nhập liệu vì RT-CONNECT luôn ánh xạ PDDx, kQ và liều/MU; không truyền giá trị rỗng để chờ Pylinac phát lỗi trong lúc đọc thuộc tính kết quả. Các giới hạn vật lý còn lại vẫn phải được kiểm theo protocol của chính lớp Pylinac và đưa thành lỗi/cảnh báo có thể hiểu được.
...
- Nuclear dùng chín lớp công khai của Pylinac: `MaxCountRate`, `PlanarUniformity`, `CenterOfRotation`, `TomographicResolution`, `SimpleSensitivity`, `FourBarResolution`, `QuadrantResolution`, `TomographicUniformity` và `TomographicContrast`. Mỗi lớp có một biểu mẫu riêng của RT-CONNECT; người dùng nhập tham số, chọn frame/vùng quan tâm hoặc hoạt độ/nuclide theo bài, còn việc phân tích và trả `results_data()` do đúng lớp Pylinac thực hiện.
- Với Nuclear, preflight chỉ nhận DICOM. Tất cả lớp nhận đúng một tệp ảnh/chuỗi theo hợp đồng, riêng `SimpleSensitivity` nhận một ảnh phantom và có thể nhận thêm một ảnh nền. Adapter không tự tính lại chỉ số; nó chỉ ánh xạ dữ liệu cấu trúc, lưu tham số đã nhập và tạo ảnh minh họa qua phương thức công khai của Pylinac nếu lớp đó hỗ trợ. Lỗi tạo ảnh minh họa không được xóa kết quả đo: run giữ kết quả và ghi cảnh báo riêng `PYLINAC_OVERLAY_UNAVAILABLE` để người dùng biết.
- Tham số Nuclear phải được kiểm tra theo bài trước khi gọi engine: thời lượng frame dương; tỷ lệ UFOV/CFOV trong khoảng hợp lệ; cửa sổ và ngưỡng dương; hoạt độ dương; nuclide thuộc danh sách đã khóa; cấu hình bốn vạch/bốn góc và sáu cầu tương ứng đúng số phần tử; frame đầu/cuối và vùng quan tâm không vượt dữ liệu. Không dùng giá trị mặc định ẩn khi giao diện đã yêu cầu người dùng nhập.
- Hai bài `contrib/One-Offs` cũng bắt buộc dùng đúng Pylinac: `QuasarLightRadScaling` nhận ảnh và dùng `results_data()` sau khi gọi `analyze(invert, fwxm, bb_edge_threshold_mm)`; `JawOrthogonality` nhận ảnh, gọi `analyze()` và lấy kết quả từ phương thức công khai `results()` vì lớp upstream không cung cấp `results_data()`. `normalize` là tùy chọn constructor của Quasar, còn Jaw không nhận tham số kỹ thuật từ giao diện. Cả hai đều lưu `source_tier=PYLINAC_CONTRIB`, phiên bản, hàm băm, tham số, kết quả và ảnh minh họa nếu tạo được.
- Pylinac contrib không tự có kết luận an toàn cuối cùng. UI chỉ hiển thị chỉ số và cho người thực hiện đánh giá riêng; lỗi ảnh không phù hợp, thiếu cạnh, không phát hiện được hình học hoặc lỗi phương thức vẽ phải trở thành lỗi/cảnh báo có mã ổn định. Không dùng bản sao thuật toán, không fallback sang engine khác và không ẩn bài nếu upstream thay đổi.
