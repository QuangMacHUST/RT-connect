...
- **Phiên bản:** 1.16 — đồng bộ specification.md v1.18, plan.md v4.4 và business-analysis.md v0.22; bổ sung record/semantics cho P17 Docker workload đồng thời và phân biệt sampled container memory với peak RSS; giữ entity/migration/API membership-invitation P4, unique pending invitation và active-context invariant, cùng reference tới feature-card/handoff, operation/error/evidence record, dependency graph, change-impact gate và status/readiness surface P20 (2026-09-09)
...
commissioning. Gate phát triển, pilot và release theo plan.md v4.4. Coordinate frame mở rộng,
...
- Benchmark local P17-W06 dùng `scripts/benchmark-p17-dvh.py` với dữ liệu DICOM tổng hợp tạm thời, mặc định `64×128×128` (`1,048,576` voxel), bốn mức dose lặp lại và ROI phủ toàn grid. Script chỉ đo elapsed time và `tracemalloc` peak, tự xóa file tạm, không ghi database/object/queue; `peak_traced_bytes` không được diễn giải thành RSS. Host evidence ghi median `1.2667533 s`, max `69,235,606` bytes; sau rebuild Docker đúng source, API container evidence ghi median `0.8206198 s`, max `69,237,022` bytes, `/health=ok`, `/ready=ready`, schema `20260909_0018`. `scripts/verify-p17-docker-workload.ps1` bổ sung local smoke 2 job đồng thời × 3 lần; 6/6 kết quả giữ cùng engine/oracle và `docker stats` có max sample `305,659,904 bytes` qua 3 mẫu, nhưng `is_peak_rss=false`. Ba evidence đều có `performance_gate=NOT_ASSESSED`. Muốn đóng performance gate phải chạy workload tương ứng trong container/worker có CPU/RAM/concurrency được pin, đo peak RSS giới hạn service (không chỉ sampled memory), kiểm API responsiveness, fault/retry và staging.
