...
| P17 | Có engine/API/UI/migration/test local; explicit P11/P16 binding, DVH report source, bounded CT renderer/overlay, local 1M-voxel volume measurement và local Docker 2-job concurrent measurement đã có evidence; staging case đã có RTDOSE/RTSTRUCT/CT và authenticated browser đã chạy saved-run/replay/refresh cùng CT overlay/no-overlap smoke trên build `fbbfa81` | Đóng P17-W05b.2/W06/W07b và các gate PostgreSQL, export byte-level, negative/fault/resource/volume, independent-oracle, release manifest. |
...
- [ ] P17-W05b.2 — Staging CT artifact upload/validation, authenticated browser overlay, warning/error matrix và direct source/hash evidence; partial staging smoke đã đạt S12–S16 và no-overlap warning với fixture tổng hợp, nhưng chỉ đóng khi hoàn tất E24–E30, direct source/hash và các assertion dữ liệu.
