...
**Đã kiểm tra cục bộ:** migration `20260913_0021` và `20260913_0022` nâng thành công; nhóm kiểm thử P5 đạt 28/28; toàn bộ kiểm thử máy chủ đạt; ruff và mypy nghiêm ngặt đạt; giao diện đạt typecheck, lint, bản dựng sản xuất và toàn bộ kiểm thử **12/12 tệp, 32/32 bài**. Cơ chế xóa vĩnh viễn lặp lại an toàn đã được kiểm ở commit `57c08e7`: lần gửi lại sau khi xóa thành công trả trạng thái đã xóa, không phát sinh lỗi giả và không dọn lan dữ liệu khác. Kiểm chứng staging danh mục/lịch sử/lưu trữ/khôi phục: [evidence P5](docs/evidence/p5-staging-authenticated-ui-20260914.json). Xóa vĩnh viễn staging và lỗi mạng/tiến trình nền vẫn mở.
