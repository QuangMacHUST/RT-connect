...
**Cập nhật trạng thái staging:** kiểm tra lỗi mạng đã đạt cho lát cắt đọc; các cổng còn mở của P5 là từ chối hồ sơ có liên kết khi thao tác thật, tiến trình nền khi chạy một lượt phân tích và đối chiếu toàn vẹn dữ liệu sau toàn bộ vòng đời.

**Cập nhật VERIFY ngày 2026-09-14:** đã hoàn tất cổng tiến trình nền bằng thao tác thật trên hồ sơ `P6 Staging Upload Smoke`. Tệp đo 3D cũ bị forced revalidation từ chối vì thiếu `coordinate_frame`; sau khi tải lại fixture hợp lệ đúng loại/vai trò, preflight chuyển sang `VALID`, worker staging nhận job và lưu run Gamma 3D `COMPLETED/PASS`, 8/8 điểm, coverage 1, P95 0, attempt 1, engine `gamma-nd-p8.2`. Bằng chứng: `docs/evidence/p5-staging-gamma-worker-20260914.json`. Việc này không đóng các cổng độ tin cậy/hiệu năng và phát hành của P8. P5 còn đúng một hành trình thao tác thật: gửi yêu cầu xóa vĩnh viễn hồ sơ liên kết `dailyQA`, xác nhận máy chủ từ chối, rồi đối chiếu 2 kết quả kiểm tra máy và 6 điểm xu hướng vẫn nguyên vẹn.
...
- [ ] P05-VERIFY — Chạy các TC-UX1 dưới đây và nhóm lỗi dùng chung có liên quan; lưu actual/evidence theo SHA. Lát cắt xóa hồ sơ không liên kết, lỗi mạng cho đường đọc và tiến trình nền Gamma thật trên staging đã PASS; còn mở yêu cầu xóa hồ sơ có liên kết và đối chiếu toàn vẹn sau khi máy chủ từ chối.
- [ ] P05-HANDOFF — Cập nhật tiến độ, dữ liệu/migration/tuyến bị tác động, giới hạn hỗ trợ và bước tiếp theo.

**Cập nhật cổng staging:** lỗi mạng và tiến trình nền Gamma thật đã đạt; còn yêu cầu từ chối hồ sơ có liên kết và đối chiếu toàn vẹn dữ liệu sau toàn bộ vòng đời.
