...
- Frontend trên candidate hiện tại: lint, typecheck, Vitest **8 passed** (trong đó `SessionErrorPage` onboarding **3/3** và Archive DVH preflight **3/3**) và production build PASS; build chỉ còn cảnh báo bundle JavaScript >500 kB, không phải lỗi functional. Evidence: `docs/evidence/p3-p4-frontend-onboarding-20260909.json` và test source `apps/web/src/pages/QAArchivePage.test.tsx`. Các dòng `1/1` ở những checkpoint lịch sử bên dưới được giữ nguyên theo candidate cũ và không đại diện cho tổng test hiện tại.
