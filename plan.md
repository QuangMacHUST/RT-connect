...
| P17 | Có engine/API/UI/migration/test local; explicit P11/P16 binding, DVH report source, bounded CT renderer/overlay, local 1M-voxel volume measurement và local Docker 2-job concurrent measurement đã có evidence; staging case đã có RTDOSE/RTSTRUCT/CT và authenticated browser đã chạy saved-run/replay/refresh, CT overlay/no-overlap smoke, export-content probe, targeted PostgreSQL row/checksum/scope probe và object-storage byte re-hash. Browser recheck hiện quan sát build `7a340b2` với cùng run/snapshot | Guard ORM + migration PostgreSQL `20260909_0019` đã deploy; schema/readiness/source SHA public smoke, trigger metadata và transaction-local negative probe PASS, không chạm run retained. Export payload JSON/CSV parse/hash PASS nhưng browser final filename vẫn UNVERIFIED; còn binding/report cloud, negative/fault/resource/volume, independent-oracle promotion và release manifest. |
...
### 7.3. Revision hiện hành v4.9

Revision hiện hành của bộ tài liệu là `business-analysis.md` v0.22, `specification.md` v1.19,
`technical-specification.md` v1.18 và `plan.md` v4.9. Revision v4.9 giữ status/readiness
...
và ghi nhận export-content parse/hash, targeted direct PostgreSQL row/checksum/scope evidence cùng object-storage byte re-hash của run DVH staging với browser finalization note.
...
`62a7b5300e39b9e742bee0b7353ff86a559d4abe`, nhưng browser/Auth/DB/audit/scope/timeout evidence
