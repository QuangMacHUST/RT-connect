...
- [ ] P17-W06 — Large workload/async execution hoặc explicit resource policy, storage/dependency fault injection, volume benchmark và staging browser E2E. `LOCAL_VOLUME_MEASURED`: synthetic 64×128×128 (`1,048,576` voxel) chạy engine `p17-dvh-1.1.0`, median `1.1705 s`, peak Python-traced allocation `69.2 MB`; evidence `docs/evidence/p17-local-volume-benchmark-20260909.json`. Chưa coi là performance gate vì chưa đo RSS/container, concurrent jobs hoặc staging.
