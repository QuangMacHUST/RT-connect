...
- [ ] P07-W04 — Xây result viewer/canvas chung: auto run, điều chỉnh bằng click/drag hoặc form, parameter diff, run mới, lớp ảnh/biểu đồ, cảnh báo của bộ tính, đánh giá và persist history. Hiện các lát cắt PF, Starshot, WL, WLMT, VMAT, FPA và FA có màn hình riêng với biểu mẫu, lịch sử, đánh giá và mở overlay; canvas dùng chung đã được mở rộng cho Starshot, Field Profile/Field Analysis ở chế độ chọn vị trí thủ công và toàn bộ Planar Imaging ở chế độ chọn tâm phantom. Ảnh overlay hiện hiển thị ngay trong trang kết quả, có thể mở/ẩn và tự gắn với đúng lượt lịch sử đang xem. Cảnh báo từ Pylinac nay hiển thị rõ trong trình xem kết quả và được giữ nguyên trong lịch sử. Tọa độ chuẩn hóa 0–1 và tọa độ điểm ảnh được chuyển đúng theo từng hợp đồng; vẫn còn canvas cho các bài chuỗi/đa ảnh, vùng ROI và lớp ảnh chuyên biệt.
