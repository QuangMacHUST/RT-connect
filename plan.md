...
**Trạng thái hiện tại:** `ACTIVE_AFTER_P06_HANDOFF`; P6 đã hoàn tất VERIFY/HANDOFF ở source `6615820`, P7 tiếp tục bằng W01/W02/W03. Đã có lát cắt local thực thi thật cho P07-PF, P07-WL, P07-STAR, P07-WLMT, ba biến thể P07-VMAT (DRGS/DRMLC/DRCS), P07-FPA và P07-FA bằng wheel đã khóa. P07-CAL và P07-LOG đã có adapter, giao diện và hợp đồng; Dynalog cùng Trajectory Log 2.1 đã chạy được dữ liệu mẫu thật của Pylinac, còn Trajectory Log 3/4 mới ở mức kiểm tra tương thích hợp đồng. Bốn lớp CatPhan 503/504/600/604, một lớp Cheese TomoCheese, một lớp Quart DVT và hai biến thể ảnh phẳng Leeds TOR/Standard Imaging FC-2 đã chạy engine thật bằng tệp mẫu chính thức; CatPhan 700, ba lớp ACR, Cheese CIRS 062M, GE Helios, Quart HyperSight và 17 biến thể ảnh phẳng còn lại vẫn cần fixture engine thật. P07-NUCLEAR hiện đã có đủ chín adapter, biểu mẫu riêng, kiểm tra đầu vào DICOM, ánh xạ `results_data()` và ảnh minh họa khi lớp Pylinac cung cấp; riêng bài tốc độ đếm cực đại có thể trả cảnh báo nếu lỗi chỉ nằm ở ảnh minh họa nhưng vẫn giữ kết quả đo. Chín bài hạt nhân chưa có fixture DICOM chuẩn và chưa kiểm chứng staging. P07-CONTRIB hiện đã có hai adapter, hai biểu mẫu, kiểm tra đầu vào ảnh, lịch sử/đánh giá và ảnh minh họa; Quasar ánh xạ `results_data()`, còn Jaw ánh xạ phương thức công khai `results()` do đặc điểm riêng của lớp. Hai bài đóng góp chưa có fixture chuẩn và chưa kiểm chứng staging. Đây chưa phải nghiệm thu P7 vì còn ma trận fixture, khung kết quả dùng chung, các bài còn thiếu và VERIFY/HANDOFF.

Bằng chứng lát cắt hiện hành: [P7 local Picket Fence và Starshot](docs/evidence/p7-local-picket-starshot-20260914.json), [P7 local Winston–Lutz](docs/evidence/p7-local-winston-lutz-20260914.json), [P7 local Winston–Lutz nhiều bi](docs/evidence/p7-local-winston-lutz-multi-target-20260914.json), [P7 local VMAT](docs/evidence/p7-local-vmat-20260914.json), [P7 local biên dạng trường](docs/evidence/p7-local-field-profile-20260914.json), [P7 hợp đồng chín bài hạt nhân](docs/evidence/p7-local-nuclear-contract-20260915.json), [P7 hợp đồng hai bài đóng góp](docs/evidence/p7-local-contrib-contract-20260915.json) và các lát cắt Log Analyzer mới. Năm bằng chứng đầu xác nhận chín capability đã gọi Pylinac thật, lưu kết quả/overlay và vượt cổng local; Log Analyzer đã xác nhận bằng dữ liệu demo Dynalog và Trajectory 2.1, còn hiệu chuẩn và các biến thể log khác vẫn cần fixture/số đo chuẩn. Lát cắt hạt nhân xác nhận đủ chín lớp, biểu mẫu và ánh xạ kết quả ở mức hợp đồng; chưa được coi là chạy engine thật cho đến khi có fixture DICOM. Lát cắt đóng góp xác nhận hai lớp, hai cách lấy kết quả công khai và biểu mẫu tương ứng; chưa được coi là chạy engine thật cho đến khi có fixture ảnh chuẩn. CatPhan, ACR, Cheese, Helios, Quart và ảnh phẳng mới có contract test/adapter, chưa có bằng chứng fixture engine thật; không cấp trạng thái hoàn thành cho các gói P7 còn lại.
...
3. Tiếp tục P07-PLANAR → P07-CT → P07-ACR → P07-CHEESE → P07-HELIOS → P07-QUART → P07-CAL → P07-LOG → P07-NUCLEAR → P07-CONTRIB; không bỏ biến thể còn công khai. P07-NUCLEAR và P07-CONTRIB đã qua cổng adapter/biểu mẫu/hợp đồng local; công việc tiếp theo của hai nhóm là fixture chuẩn, đối chiếu trực tiếp từng lớp và staging.
...
- [ ] P07-QUART — Quart DVT và alias/biến thể HyperSight còn public trong wheel; HU/geometry/uniformity/CNR/SNR và module controls. Lát cắt hiện đã nối `QuartDVT` và `HypersightQuartDVT`, có dung sai HU/thang đo/độ dày/CNR, vị trí lát và dịch lát tìm góc. Quart DVT đã chạy bằng `quart.zip`, trả 10 nhóm chỉ số và overlay; HyperSight còn thiếu fixture để xác nhận alias/biến thể. Còn thiếu đối chiếu module, kiểm thử lỗi đầy đủ và staging.
