...
| P17 | Có engine/API/UI/migration/test local; explicit P11/P16 binding, DVH report source, bounded CT renderer/overlay, local 1M-voxel volume measurement và local Docker 2-job concurrent measurement đã có evidence; staging case đã có RTDOSE/RTSTRUCT/CT và authenticated browser đã chạy saved-run/replay/refresh, CT overlay/no-overlap smoke, export-content probe, targeted PostgreSQL row/checksum/scope probe và object-storage byte re-hash trên build `4b6423e5` | Đóng P17-W05b.2/W06/W07b và các gate immutable snapshot mutation guard, browser export finalization, negative/fault/resource/volume, independent-oracle promotion, release manifest. |
...
- [ ] P17-W05b.2 — Staging CT artifact upload/validation, authenticated browser overlay, warning/error matrix và direct source/hash evidence; partial staging smoke đã đạt S12–S16, no-overlap warning, export-content parse/hash, targeted PostgreSQL row/checksum/scope probe, object-storage byte re-hash và snapshot-field recheck với fixture tổng hợp, nhưng chỉ đóng khi hoàn tất E24–E30, immutable-snapshot mutation guard và các assertion dữ liệu còn lại.
...
- **Staging exit:** đúng candidate với migration P17 `20260908_0017` đã nằm trong schema head hiện hành `20260909_0018`; browser→API→Railway PostgreSQL/object storage; direct DB row/hash/scope và object-storage byte re-hash; positive/negative/idempotency/reconnect/export; fault injection và workload trong budget.
