...
- [ ] P07-PLANAR — Toàn bộ Planar Imaging: Leeds/Blue, SI QC-3/QC-kV, Las Vegas/Elekta, Doselab MC2 MV/kV, SNC MV/MV12510/kV, PTW EPID QC, IBA Primus A, SI FC-2, IMT L-RAD, Doselab RLf, IsoAlign, SNC FSQA, ACR Digital Mammography. Lát cắt hiện đã nối đủ 19 lớp Pylinac, kiểm tra đúng một ảnh, truyền tham số phantom/tâm/góc/vùng quan tâm, lưu overlay/lịch sử và có giao diện chung; chưa có fixture thật cho từng biến thể, chưa đối chiếu structured result/overlay và chưa kiểm chứng staging.
- [ ] P07-FPA — Field Profile Analysis: manual/beam/geometric center, vị trí và độ rộng x/y, normalization, edge type và toàn bộ profile metrics công khai. Lát cắt local đã gọi `FieldProfileAnalysis.analyze()`/`results_data()`/`plot_analyzed_images()`, có biểu mẫu tham số, lịch sử, đánh giá và ảnh phân tích; còn thiếu canvas click/drag, đầy đủ dữ liệu biên dạng và kiểm chứng staging.
- [ ] P07-FA — Field Analysis legacy: UI tương thích EPID/SNC Profiler và protocol cũ; nhãn mô-đun cũ, bài mới mặc định FPA nhưng không loại capability khỏi catalog. Lát cắt local đã gọi `FieldAnalysis.analyze()`/`results_data()`/`plot_analyzed_image()`, có lựa chọn protocol, nội suy, tâm, chuẩn hóa và ảnh phân tích; còn thiếu ma trận fixture legacy đầy đủ, canvas và kiểm chứng staging.
...
- TC-UX1-P07-S16 — P07-FPA: vị trí profile đồng bộ ô số, centering/width/edge/normalization/metrics được truyền đúng và biểu đồ khớp structured result; thao tác click sẽ dùng cùng canvas khi W04 hoàn tất.
