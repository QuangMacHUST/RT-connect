...
| P17 | Có engine/API/UI/migration/test local; explicit P11/P16 binding, DVH report source, bounded CT renderer/overlay và local 1M-voxel volume measurement đã có evidence; chưa có staging saved-run/CT E2E, binding/report cloud evidence, container RSS/fault/independent-oracle evidence | Đóng P17-W05b.2/W06/W07b và P17 staging/volume/fault/release gates. |
...
- [ ] P17-W06 — Large workload/async execution hoặc explicit resource policy, storage/dependency fault injection, volume benchmark và staging browser E2E. `LOCAL_VOLUME_MEASURED`: synthetic 64×128×128 (`1,048,576` voxel) chạy engine `p17-dvh-1.1.0`, median `1.2667533 s`, peak Python-traced allocation `69,235,606 bytes`; evidence `docs/evidence/p17-local-volume-benchmark-20260909.json`. Chưa coi là performance gate vì chưa đo RSS/container, concurrent jobs hoặc staging.
