...
| P4–P5 | CRUD hierarchy/folder/case có staged smoke; P4 invitation/member slice đã deploy staging, readiness đã lên schema `20260909_0018` và label parity đã pass trên candidate `311abed` | P4 Auth/two-identity browser flow, DB hash/status/audit, active-context/last-member/concurrency/timeout; P5 restore, simultaneous edits, complete filter/history là gate bổ sung cần kiểm/triển khai. |
...
| P17 | Có engine/API/UI/migration/test local; explicit P11/P16 binding, DVH report source và bounded CT renderer/overlay đã có local evidence; chưa có staging saved-run/CT E2E, binding/report cloud evidence, volume/fault/oracle | Đóng P17-W05b.2/W06/W07b và P17 staging/volume/fault/release gates. |
...
- **Checkpoint staging mới nhất (2026-09-09):** candidate `311abed` đã được deploy trên API/web/worker staging; API `/api/v1/ready` trả `200` với schema `20260909_0018`, `/api/v1/version` trả version `311abed`, OpenAPI đã có member/invitation routes, và các route mới không có Auth trả `401`. Public evidence hiện hành `docs/evidence/p4-staging-public-smoke-20260909.json` đạt **14/14** và version parity đã pass. Auth browser với hai identity, PostgreSQL row/hash/status/audit, replay/revoke/expiry/context/concurrency/timeout vẫn mở.
...
- [x] P17-VERIFY-LOCAL — `test_dvh_engine.py` **13 passed**, `test_dvh.py` **6 passed**, `test_reports.py` **4 passed**, `test_biological_library.py` **3 passed** (focused P17/P16/report/library **26 passed**); full backend **168 passed**, Ruff, strict mypy, frontend lint/typecheck/Vitest/build và migration/OpenAPI checks đã pass trên working tree candidate. Các case mới bao phủ explicit P11/P16 binding, missing/invalid/mismatched manifest checksum, CT Frame of Reference mismatch, CT no-overlap read-only preview, CT resource guard, changed CT bytes và Compose schema-revision type regression.
...
`scripts/verify-public-deployment.ps1` đã được chạy với timeout hữu hạn và evidence được lưu tại `docs/evidence/p4-staging-public-smoke-20260909.json`. Kết quả **14/14 checks PASS** với health HTTP 200, readiness HTTP 200, schema head `20260909_0018`, API/web version `311abed`, OpenAPI chứa CT preview/member/invitation routes, ba route membership/invitation thực tế trả `401` khi chưa xác thực, web index/bundle HTTP 200 và bundle chứa CT preview cùng UI membership/invitation. Đây là public boundary và version-parity evidence, không phải Authenticated E2E, direct PostgreSQL evidence, DVH run hoặc production release evidence. Auth/two-identity, persistence/audit, DVH và production manifest vẫn là gate riêng.
...
Evidence staging lịch sử: `docs/evidence/p19-staging-public-smoke-20260909.json` — version `65dd52b`, schema `20260908_0017`, 10/10 checks pass. Evidence public hiện tại: `docs/evidence/p4-staging-public-smoke-20260909.json` — version `311abed`, schema `20260909_0018`, 14/14 checks pass; version parity staging đã đạt. Evidence lịch sử không được dùng thay cho Authenticated E2E, direct database, rollback hoặc production manifest.
...
| P17 | `S01–S16` | `E01–E30` | B01, B02, B04, B05, B06, B09, B10, B11, B12 | D, A, DB, UI, R, V, P |
...
| **P17** | Resolve dose/structure/CT → preflight geometry/checksum → dose-native/overlay → ROI/coverage → optional explicit P11/P16 binding → validate → save DVH → history/export/report; CT preview là operation read-only riêng. | `S01–S16`, dose-only, ROI by ROINumber, FULL/OVERLAP policy, bounded CT HU/window/frame/LPS overlay, DVH oracle, actual/limit/margin và report source. | `E01–E30`, C03–C16 plus geometry/volume/binding: manifest/source/grid/frame/ROI/contour/coverage/CT/resource/unit/protocol/idempotency/checksum/no-overlap. | Schema `20260908_0017`, engine/binding/result hash, CT preview hash/mapping, ROI/coverage/limit metrics, DB/object/scope, browser run/report. | Không đóng nếu staging saved-run/CT/binding/report, independent oracle hoặc volume/fault gate còn mở; bounded CT phải có S12–S16/E24–E30; mở P18 khi release scope ghi rõ. |
