...
- [ ] P07-W04 — Xây result viewer/canvas chung: auto run, điều chỉnh bằng click/drag hoặc form, parameter diff, run mới, lớp ảnh/biểu đồ, cảnh báo của bộ tính, đánh giá và persist history. Hiện các lát cắt PF, Starshot, WL, WLMT, VMAT, FPA và FA có màn hình riêng với biểu mẫu, lịch sử, đánh giá và mở overlay; canvas dùng chung đã được mở rộng cho Picket Fence, Starshot, Field Profile/Field Analysis ở chế độ chọn vị trí thủ công và toàn bộ Planar Imaging ở chế độ chọn tâm phantom. Vùng chọn điểm hiện dùng tuyến xem trước có xác thực, dựng DICOM/ZIP thành PNG an toàn trong phạm vi tổ chức, có tuyến thông tin số ảnh/lát; chuỗi ngắn dùng danh sách, chuỗi dài dùng ô nhập số trong phạm vi tối đa 2.048 ảnh/lát nên không phụ thuộc vào trình duyệt tự hiển thị DICOM; Picket Fence có thể chọn hoặc nhập tâm trục trung tâm trước khi gọi Pylinac. Ảnh overlay hiện hiển thị ngay trong trang kết quả, có thể mở/ẩn và tự gắn với đúng lượt lịch sử đang xem. Cảnh báo từ Pylinac hiển thị trong vùng riêng, không tự đổi đánh giá và được giữ đúng theo lượt lịch sử; hồi quy cục bộ đạt 2/2, toàn bộ giao diện đạt 53/53. Tọa độ chuẩn hóa 0–1 và tọa độ điểm ảnh được chuyển đúng theo từng hợp đồng. **Cập nhật 2026-09-16:** trình xem Winston–Lutz nhiều bi đã có bảng đối chiếu theo từng ảnh và từng bi, lấy trực tiếp từ `image_details` của Pylinac; hiển thị góc, khoảng cách trường–bi và sai lệch xoay bàn, không hiển thị tên tệp kỹ thuật và hoạt động cả khi mở lại lượt cũ. Kiểm thử giao diện riêng đạt 8/8. Vẫn còn ROI chuyên biệt, ánh xạ tọa độ thao tác theo từng ảnh, lựa chọn ảnh ngoài giới hạn xem trước và kiểm chứng tương tác staging.
  ...
  - **Bổ sung cổng đầu vào 2026-09-16:** mọi bài Pylinac có tệp chỉ được phân tích khi các tệp đang chọn có trạng thái `VALID`; bài nhiều tệp kiểm tra toàn bộ danh sách, tệp cảnh báo vẫn có thể kiểm tra lại và bài hiệu chuẩn nhập số liệu không bị ảnh hưởng. Kiểm thử giao diện đạt `56/56`, kiểm tra kiểu, lint, bản dựng và hợp đồng kế hoạch đạt. Bằng chứng: `docs/evidence/p7-local-validated-input-gate-20260916.md`. Chưa đóng kiểm chứng staging toàn bộ bài, fixture chuẩn hoặc nghiệm thu lâm sàng.
   ...
   - **Kiểm tra staging 2026-09-16:** API, worker và giao diện đã cùng phục vụ commit `02c154238dbaac047634f50ff31e4638f9b6eda9`; cổng exact-SHA công khai đạt `16/16`, readiness giữ lược đồ `20260914_0023`, tuyến xem trước và ranh giới xác thực đều đạt. Đây là bằng chứng parity triển khai, chưa phải chạy phantom được phê duyệt, đối chiếu độc lập hoặc nghiệm thu P7. Bằng chứng: `docs/evidence/p7-staging-visual-origin-slice-deployment-20260916.json`.
