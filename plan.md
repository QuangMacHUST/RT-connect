...
| P17 | Có engine/API/UI/migration/test local; explicit P11/P16 binding, DVH report source, bounded CT renderer/overlay, local 1M-voxel volume measurement và local Docker 2-job concurrent measurement đã có evidence; staging case đã có RTDOSE/RTSTRUCT/CT và authenticated browser đã chạy saved-run/replay/refresh, CT overlay/no-overlap smoke, export-content probe, targeted PostgreSQL row/checksum/scope probe và object-storage byte re-hash trên build `4b6423e5` | Guard ORM + migration PostgreSQL `20260909_0019` đã deploy; schema/readiness/source SHA, trigger metadata và transaction-local negative probe PASS, không chạm run retained. Còn browser export finalization, binding/report cloud, negative/fault/resource/volume, independent-oracle promotion và release manifest. |
...
- **Trạng thái hiện tại:** `STAGING_DVH_CT_BROWSER_DIRECT_DATA_PROBES_PASS_GATES_OPEN`; engine/API/UI/migration/test local đã có, explicit P11/P16 limit binding, DVH report source và bounded CT renderer đã có code/test local. Migration P17 `20260908_0017` và guard bất biến `20260909_0019` đã được triển khai; `/ready`/`/version` match candidate `d04fb5a3561804bd3553b36886902e1c50b2f18f` với schema `20260909_0019`, trigger `trg_dvh_analysis_runs_immutable` tồn tại và transaction-local negative probe đã từ chối UPDATE/DELETE với `DVH_RUN_IMMUTABLE`, không chạm run retained. Case smoke `8bc86303-c7e9-4e1a-b012-cfbe2a07ba24` hiện có RTDOSE GY hợp lệ, RTSTRUCT và CT hợp lệ; browser đã xác nhận preflight, ROI tự nạp, DVH saved-run/replay/refresh, CT frame `#1` overlay, CT frame `#3` no-overlap warning và export-content parse/hash. Read-only PostgreSQL query trong API container đã xác nhận 3 artifact rows, 3 input manifests, 1 DVH run, scope organization/case và manifest-to-artifact checksum; object-storage byte re-hash và snapshot-field recheck cũng PASS. Browser finalization, binding/report cloud, fault/resource/volume, full negative matrix, independent-oracle promotion và release evidence chưa đóng.
