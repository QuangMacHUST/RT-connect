...
| P17 | Có engine/API/UI/migration/test local; explicit P11/P16 binding, DVH report source, bounded CT renderer/overlay, local 1M-voxel volume measurement và local Docker 2-job concurrent measurement đã có evidence; staging case đã có RTDOSE/RTSTRUCT/CT và authenticated browser đã chạy saved-run/replay/refresh, CT overlay/no-overlap smoke cùng export-content probe trên build `4b6423e5` | Đóng P17-W05b.2/W06/W07b và các gate PostgreSQL, browser export finalization, negative/fault/resource/volume, independent-oracle, release manifest. |
...
- [ ] P17-W05b.2 — Staging CT artifact upload/validation, authenticated browser overlay, warning/error matrix và direct source/hash evidence; partial staging smoke đã đạt S12–S16, no-overlap warning và export-content parse/hash với fixture tổng hợp, nhưng chỉ đóng khi hoàn tất E24–E30, direct source/hash và các assertion dữ liệu.
...
- [ ] P17-VERIFY-STAGING — migration P17 `20260908_0017` trong schema head staging `20260909_0018`, readiness/schema, authenticated browser input→validate→save→refresh→export, PostgreSQL row/checksum/scope, negative/fault/volume evidence. Browser input→validate→save→refresh, CT frame `#1/#3` và export-content parse/hash đã được xác nhận; browser finalization, direct PostgreSQL/hash/scope, fault/resource/volume và release evidence còn mở.
...
và ghi nhận export-content parse/hash evidence của run DVH staging với browser finalization note.
