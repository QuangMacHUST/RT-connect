...
- [x] P17-VERIFY-LOCAL — `test_dvh_engine.py` **14 passed**, `test_dvh.py` **6 passed**, `test_reports.py` **4 passed**, `test_biological_library.py` **3 passed** (focused P17/P16/report/library **27 passed**); full backend **169 passed**, Ruff, strict mypy, frontend lint/typecheck/Vitest/build và migration/OpenAPI checks đã pass trên commit `7377aad`. Các case mới bao phủ explicit P11/P16 binding, missing/invalid/mismatched manifest checksum, CT Frame of Reference mismatch, CT no-overlap read-only preview, CT resource guard, changed CT bytes, Compose schema-revision type regression và volume-weighted DVH với nonuniform slice thickness. D95 known-answer được pin ở `5.20 Gy` theo cumulative-DVH convention; evidence: `docs/evidence/p17-local-volume-weighted-dvh-20260909.json`.
