...
- [ ] P08-W03 — Queue status/retry/cancel và kết quả 1D/2D gồm map/profile, histogram, pass rate, evaluated/excluded/invalid counts và cảnh báo vùng không đánh giá. Lát cắt local hiện đã có tuyến hủy trước khi worker nhận việc, trạng thái `CANCELLED`, nhật ký hủy, cập nhật outbox và worker bỏ qua thông điệp cũ; kiểm thử Gamma đạt 15/15 theo [bằng chứng hàng đợi và kết quả Gamma cục bộ](docs/evidence/p8-local-gamma-queue-results-20260918.md). Vẫn cần kiểm chứng queue thật trên staging cùng toàn bộ ma trận lỗi.
  ...
  Cổng đầu vào một chiều đã được sửa và kiểm tra cục bộ: tệp đo liều `gamma.measurement.v1` có lưới `[n]` và trục `x` nay được xác thực đúng, hình học hai hồ sơ một chiều được chấp nhận trước khi đưa vào hàng đợi. Bằng chứng: [đầu vào Gamma một chiều Pylinac cục bộ](docs/evidence/p8-local-gamma-1d-pylinac-20260918.md). Đây chưa phải bằng chứng staging; cần một cặp dữ liệu đo liều một chiều tổng hợp đã xác thực để chạy thật.
