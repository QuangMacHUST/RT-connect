...
- [ ] P07-W03 — Phát triển `PylinacCapabilityRegistry` + `PylinacAdapter`; inventory wheel phải khớp registry đủ 16 họ chính, QA contrib công khai, class/variant, input profile, parameter schema, result mapping, overlay, error và fixture. Lát cắt local hiện đã có API, migration, lưu input/result/overlay và adapter thật cho Picket Fence, Winston–Lutz, Starshot, Winston–Lutz nhiều bi, ba lớp VMAT DRGS/DRMLC/DRCS, Field Profile Analysis, Field Analysis legacy, năm lớp hiệu chuẩn TG-51/TRS-398, bốn lớp Log Analyzer, chín lớp Nuclear và hai lớp Contrib. Dynalog và Trajectory Log 2.1 đã chạy dữ liệu mẫu thật; Trajectory Log 3/4 mới có contract/version test. CatPhan 503/504/600/604, TomoCheese, Quart DVT, nhãn HyperSight qua bộ tính QuartDVT tương thích và toàn bộ 19 biến thể ảnh phẳng đã chạy được tệp mẫu chính thức; CatPhan 700, ACR CT/MRI, CIRS 062M, GE Helios, chín bài Nuclear và hai bài Contrib vẫn cần fixture engine thật. Ma trận fixture, đối chiếu kết quả, kiểm lỗi đặc trưng và staging của các nhóm còn lại vẫn mở, nên W03 chưa hoàn thành.
