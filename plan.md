...
**Đã kiểm tra cục bộ:** migration `20260913_0021` và `20260913_0022` nâng thành công; nhóm kiểm thử P5 hiện đạt 31/31; toàn bộ kiểm thử máy chủ đạt **218/218**; ruff và mypy nghiêm ngặt đạt; giao diện đạt typecheck, lint, bản dựng sản xuất và toàn bộ kiểm thử **12/12 tệp, 37/37 bài**. Bằng chứng hồi quy mới nhất: [P5 local regression](docs/evidence/p5-local-regression-20260914-99d68f6.json). Bài kiểm thử bổ sung cho báo cáo xác nhận hồ sơ đã lưu trữ nhưng còn báo cáo trỏ tới bị từ chối xóa và vẫn đọc được; bằng chứng: [bảo vệ tham chiếu báo cáo](docs/evidence/p5-local-report-reference-guard-20260914.json). Bài kiểm thử bổ sung ngày 2026-09-14 xác nhận hồ sơ đã lưu trữ nhưng còn tệp đầu vào cũng bị từ chối xóa và vẫn đọc được; bằng chứng: [bảo vệ tệp đầu vào](docs/evidence/p5-local-input-artifact-purge-guard-20260914.json). Bước xem trước xóa chỉ đọc đã được triển khai ở commit `a2245f1`: hồ sơ có báo cáo trả đúng trạng thái không đủ điều kiện và không gửi yêu cầu xóa; hồ sơ đủ điều kiện hiển thị cơ sở, máy và thời điểm trước khi hỏi xác nhận. Bằng chứng: [xem trước xóa cục bộ](docs/evidence/p5-local-purge-preview-20260914.json). Cơ chế xóa vĩnh viễn lặp lại an toàn đã được kiểm ở commit `57c08e7`: lần gửi lại sau khi xóa thành công trả trạng thái đã xóa, không phát sinh lỗi giả và không dọn lan dữ liệu khác. Kiểm thử bổ sung xác nhận hồ sơ có tác vụ Gamma đang chờ hoặc đang chạy đều bị từ chối xóa. Kiểm thử toàn vẹn mới xác nhận xóa một hồ sơ không liên kết không làm mất hồ sơ khác trong cùng thư mục hoặc làm mất thư mục chứa. Trang chi tiết nhập số đo đã được Việt hóa và ẩn mã lượt/chỉ số/cấu hình thô ở commit `aaada4e`. Kiểm chứng staging danh mục/lịch sử/lưu trữ/khôi phục: [bằng chứng P5](docs/evidence/p5-staging-authenticated-ui-20260914.json). Lát cắt xóa vĩnh viễn hồ sơ thử nghiệm riêng không liên kết đã PASS: [bằng chứng xóa staging](docs/evidence/p5-staging-unreferenced-purge-20260914.json). Thao tác nhiều mục đã có kiểm thử cục bộ và kiểm tra chọn/bỏ chọn trên staging: [bằng chứng chọn nhiều staging](docs/evidence/p5-staging-batch-selection-20260914.json). Recheck parity staging sau cơ chế mốc theo dõi đạt **16/16**, API/web/worker cùng bản `db97b0e`, cùng lược đồ `20260913_0022`; bằng chứng: [P5 staging parity PASS](docs/evidence/p5-staging-public-recheck-20260914-db97b0e.json). Các trường hợp từ chối dữ liệu có liên kết, lỗi mạng và tiến trình nền khi thao tác thật trên staging vẫn mở.
