...
| P17 | Có engine/API/UI/migration/test local; explicit P11/P16 binding, DVH report source, bounded CT renderer/overlay, local 1M-voxel volume measurement và local Docker 2-job concurrent measurement đã có evidence; staging case đã có RTDOSE/RTSTRUCT/CT và authenticated browser đã chạy saved-run/replay/refresh, CT overlay/no-overlap smoke cùng export-content probe trên build `fbbfa81` | Đóng P17-W05b.2/W06/W07b và các gate PostgreSQL, browser export finalization, negative/fault/resource/volume, independent-oracle, release manifest. |
