...
## 22. Ma trận hành vi ở cấp tính năng v0.18
...
| **P17** `FR-P17-01..04`<br>Visual Dose/DVH | Chọn case → resolve VALID RTDOSE/RTSTRUCT/optional CT → geometry/manifest/checksum preflight → dose-native hoặc anatomy overlay → chọn ROI/policy/metric → validate preview → save DVH run → history/export/report explicit. | Dose-native chạy không cần CT; DVH dùng ROI theo ROINumber; FULL_ROI/OVERLAP_ONLY explicit; Dmin/Dmean/Dmax/Dx/Vx và weighted volume cùng snapshot; CT overlay chỉ bật khi frame/hình học compatible. | Thiếu/wrong artifact, manifest/checksum drift, unit/scaling/grid/frame/orientation/ROI/contour lỗi, coverage thiếu, metric/resource, idempotency/persistence: chặn hoặc warning theo policy, không clip/gán zero/đoán transform; retry sau reconcile, dose-only fallback có nhãn. | Input/result checksum, geometry/ROI/coverage, engine/schema version, result hash, JSON/CSV và report source. `DVH_INPUT_NOT_VALIDATED`, `DVH_SOURCE_CHANGED`, `DICOM_GEOMETRY_INVALID`, `DVH_ROI_INVALID`, `CONTOUR_GEOMETRY_INVALID`, `DVH_INCOMPLETE_COVERAGE`, `DVH_PARTIAL_COVERAGE`, `DVH_RESOURCE_LIMIT`. |
