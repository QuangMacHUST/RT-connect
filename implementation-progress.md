...
- P4 đã đạt `STAGING_VERIFIED_SLICE` cho lát cắt giao diện Đơn vị và thiết bị: thẻ đơn vị, cơ sở/máy cạnh nhau, thành viên/lời mời thu gọn; lint, typecheck, Vitest **27/27**, production build PASS, ba dịch vụ staging cùng mã `5432bc2b2598eea7d6edc42fb7065a22d3f3ccf5`, public verifier trực tiếp **16/16**. Biểu mẫu tạo máy chỉ yêu cầu tên hiển thị và máy chủ tự tạo mã nội bộ; lỗi nghiệp vụ P4 được dịch sang tiếng Việt; biểu mẫu cơ sở/máy/lời mời giữ lại nội dung khi máy chủ trả lỗi; lưu trữ cơ sở/máy phải được xác nhận trước và nêu rõ lịch sử không bị xóa. API P4 cục bộ đạt **17/17** kiểm thử tổ chức, trong đó có kiểm thử máy lưu trữ giữ bài QA cũ và chặn bài mới; nhóm kiểm thử tổ chức + kho QA đạt **20/20**. Đã bổ sung kiểm thử giao diện hủy lưu trữ máy và [phiếu kiểm chứng staging hai tài khoản](docs/runbooks/p4-kiem-chung-hai-tai-khoan-staging.md), trong đó nêu rõ không dùng chung phiên, không ghi mã mời/mật khẩu và không đóng P4 nếu thiếu tài khoản B. Evidence: [P4 local UI](docs/evidence/p4-local-organization-ui-20260913-8a1fe44.json), [P4 local API](docs/evidence/p4-local-organization-api-20260913.json), [P4 local machine creation](docs/evidence/p4-local-machine-creation-20260913.json), [P4 local QA history](docs/evidence/p4-local-qa-history-20260913.json), [P4 staging](docs/evidence/p4-staging-verified-20260913-8a1fe44.json), [P4 public verifier trước](docs/evidence/p4-staging-public-verifier-20260913-e8c4323.json), [P4 public verifier giữa kỳ](docs/evidence/p4-staging-public-verifier-20260913-9d95f89.json), [P4 public verifier cũ](docs/evidence/p4-staging-public-verifier-20260913-b53916c.json), [P4 public verifier đã lưu](docs/evidence/p4-staging-public-verifier-20260913-d329ab7.json). Sau khi commit tài liệu `5432bc2`, kiểm parity trực tiếp vẫn đạt 16/16 với mã `5432bc2`; P4 vẫn mở để kiểm hai danh tính thật, bộ chọn bài QA và lịch sử trên staging.
- Sau khi rà soát trải nghiệm thực tế, trang QA máy đã được tinh giản ở commit mới nhất của nhánh: toàn bộ nhãn hiển thị là tiếng Việt, không còn hiện mã bài/tệp, mã kiểm tra tệp hoặc trạng thái kỹ thuật thô; loại bài kiểm tra và chu kỳ dùng nhãn thân thiện nhưng vẫn gửi đúng giá trị nội bộ. Luồng kiểm tra tệp và phân tích liều cũng đã được Việt hóa; các trạng thái bài kiểm tra `OPEN`, `IN_REVIEW` và `CANCELLED` cũng đã có nhãn tiếng Việt. Kiểm thử giao diện đạt **27/27**, lint/typecheck/build PASS; sau khi đồng bộ API, web và tiến trình nền staging, public verifier đạt **16/16** với cùng commit và schema `20260911_0020`. Đây là sửa lỗi trải nghiệm của lát cắt P4, không đóng P4; kiểm thử hai danh tính thật và bộ chọn/lịch sử trên staging vẫn còn mở.
- Đã sửa luồng tài khoản chưa có đơn vị: máy chủ có tuyến tra cứu lời mời đang chờ theo email Supabase đã xác thực, giao diện tự hiển thị tên đơn vị và nút “Tham gia”, đồng thời có ô dán mã mời dự phòng. Mã bản ghi nội bộ không hiển thị; thao tác tham gia không đi qua tuyến tạo đơn vị. Kiểm thử mới: API tổ chức **18/18**, giao diện **29/29**, lint/typecheck/build PASS. Kiểm trực tiếp staging đã đạt P4-S03: lời mời hiển thị, tham gia thành công, đúng đơn vị và readback **2 thành viên**; bằng chứng: [hộp thư lời mời staging](docs/evidence/p4-staging-invite-inbox-20260913-246d1e5.json). P4 vẫn chưa hoàn tất vì còn kiểm mã dự phòng, hai phiên độc lập, vòng đời máy/lịch sử và bàn giao.
