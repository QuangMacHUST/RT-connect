...
- **Local evidence:** focused suite `24 passed` (13 DVH engine + 4 DVH API workflows + 4 report tests + 3 Biological Library tests), full backend suite `165 passed`, Ruff and strict mypy pass; frontend lint/typecheck/Vitest pass. Added coverage includes P16 D95 binding, P11 lowercase rule lookup, binding conflict, missing metric, invalid override, and fail-closed DVH input discovery/validation when the manifest is pending, invalid, or does not match the artifact checksum.
