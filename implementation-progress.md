...
- **Current phase:** P17 — Visual Dose, DVH và structure review; API/worker functional candidate và fixture/test descendant `65dd52b` đã deploy staging thành công; web đã nhận descendant `03802e7` để bổ sung dashboard đọc tách health/readiness/version/queue. Không giả định ba service cùng SHA khi chỉ web thay đổi. Browser E2E P17 vẫn chờ bổ sung RTSTRUCT/CT tổng hợp vào QA case. P16 browser smoke đã pass nhưng database/scope/fault/release closure còn mở, cùng các gate P8/P9/P10/P11/P12/P13/P14/P15 và các phase tích hợp sau.
- **Current status:** IN_PROGRESS — P17 local slice đã có pure RTDOSE/RTSTRUCT engine, migration `20260908_0017`, API inputs/validate/save/history/export, organization/case scope, checksum/idempotency snapshot, explicit P11/P16 limit binding, DVH report source, bounded CT preview/overlay và route `/app/qa/cases/:caseId/dvh`. CT preview đã decode HU, window/level, chọn frame, map dose/ROI nearest-neighbor trong patient LPS, crosshair, mapping metadata, resource guard và deterministic hash; đây là read-only visual operation, không tạo DVH run. API/worker commit `65dd52bec5f692f92d6f6dddc3df485aa6cce883` đã deploy thành công; web dashboard commit `03802e7` đã build và phục vụ staging. `/api/v1/ready` trả `schema_revision=20260908_0017`, `/api/v1/version` trả `version=65dd52b`, OpenAPI public đã expose `/dvh/ct-preview`, web bundle public đã chứa CT preview controls và marker `Schema readiness`. Đây là evidence staging; production promotion/release manifest vẫn mở. Staging case hiện có RTDOSE hợp lệ nhưng chưa có RTSTRUCT/CT nên nút tính và CT overlay vẫn chờ input. Staging DVH/CT/binding/report E2E, fault/volume/release evidence chưa đóng. P16/P15 replay/DB/scope gates vẫn mở độc lập.
- **Last authoritative check:** 2026-09-09 — local P17 engine focused suite `13 passed`; combined DVH engine/API/report source suite `20 passed`; P18 integrated journey suite `2 passed`; full backend suite `155 passed`; P20 status-dashboard test `2 passed`; Ruff/mypy, frontend lint/typecheck/Vitest/build, migration/OpenAPI pass. Railway/public checks xác nhận API `/api/v1/health`, `/api/v1/ready`, `/api/v1/version`, `/api/v1/openapi.json` và web root đều trả thành công; web bundle staging mới chứa `Schema readiness`. Authenticated browser staging `/app/system/status` hiển thị `API HEALTH=OK`, `SCHEMA READINESS=READY`, `schema_revision=20260908_0017`, `API VERSION=65dd52b` và `redis_stream=available/configured`, với queue metrics hiện tại queued/running/retrying/failed đều bằng 0. API/worker vẫn được ghi theo source `65dd52b`; web dashboard theo source `03802e7`. RTSTRUCT `docs/fixtures/p17-rtstruct-v1-smoke.dcm` có local oracle `FULL_ROI`, 4 voxels, mean `6.5 Gy`, D95 `5.15 Gy`, SHA-256 `16a79df3129757d9df8b48bd095f0b4b70b24713ea5255e24719d46e6808d401`; CT `docs/fixtures/p17-ct-v1-smoke.dcm` có SHA-256 `0b1d3b3fd6adf33f139e493ca043e09defe077cab24d57287a6076816315dc088`, frame 1 overlay oracle và frame 2 no-overlap oracle. Storage outage negative test trả `DVH_STORAGE_UNAVAILABLE` HTTP 503 và xác nhận không tạo run.
