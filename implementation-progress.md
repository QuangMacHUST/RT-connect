...
- **Current phase:** P17 — Visual Dose, DVH và structure review; candidate `893ae2d` đã deploy staging đúng source SHA và schema nhưng browser E2E còn chờ bổ sung RTSTRUCT/CT tổng hợp vào QA case. P16 browser smoke đã pass nhưng database/scope/fault/release closure còn mở, cùng các gate P8/P9/P10/P11/P12/P13/P14/P15 và các phase tích hợp sau.
- **Current status:** IN_PROGRESS — P17 local slice đã có pure RTDOSE/RTSTRUCT engine, migration `20260908_0017`, API inputs/validate/save/history/export, organization/case scope, checksum/idempotency snapshot, explicit P11/P16 limit binding, DVH report source, bounded CT preview/overlay và route `/app/qa/cases/:caseId/dvh`. CT preview đã decode HU, window/level, chọn frame, map dose/ROI nearest-neighbor trong patient LPS, crosshair, mapping metadata, resource guard và deterministic hash; đây là read-only visual operation, không tạo DVH run. Railway API/web/worker của source SHA `893ae2d46c197b80fc78e29cdea25ab19a54154c` đều `SUCCESS`; `/api/v1/ready` trả `schema_revision=20260908_0017`, OpenAPI public đã expose `/dvh/ct-preview`, web bundle public đã chứa CT preview controls. `/api/v1/version` vẫn trả build label `9262bfd`, nên source SHA và application build label chưa phải cùng một định danh; đây là release-metadata gap phải xử lý trước P19. Staging case hiện có RTDOSE hợp lệ nhưng chưa có RTSTRUCT/CT nên nút tính và CT overlay vẫn chờ input. Staging DVH/CT/binding/report E2E, fault/volume/release evidence chưa đóng. P16/P15 replay/DB/scope gates vẫn mở độc lập.
- **Last authoritative check:** 2026-09-09 — local P17 engine/API/CT focused suite `13 passed`; full backend suite `150 passed`; Ruff/mypy, frontend lint/typecheck/Vitest/build, migration/OpenAPI pass. Railway metadata xác nhận API deployment `c5807acb-dd49-4279-8304-ecc95141543d`, web `b46d7617-8545-40dc-82b6-d2f42103d696`, worker `3b0b89e3-b633-4477-9bda-48d01af2f3be`, tất cả `SUCCESS`, cùng source SHA `893ae2d46c197b80fc78e29cdea25ab19a54154c`; API `/api/v1/health`, `/api/v1/ready`, `/api/v1/version`, `/api/v1/openapi.json` và web root đều trả thành công, OpenAPI có `ct-preview`, web bundle mới có CT controls. Fixture `docs/fixtures/p17-rtstruct-v1-smoke.dcm` được kiểm tra local với `FULL_ROI`, 4 voxels, mean `6.5 Gy`, D95 `5.15 Gy`, SHA-256 `16a79df3129757d9df8b48bd095f0b4b70b24713ea5255e24719d46e6808d401`.
- **Next exact step:** sau khi user xác nhận thao tác chọn file, upload fixture RTSTRUCT tổng hợp và một CT fixture hợp lệ vào case `8bc86303-c7e9-4e1a-b012-cfbe2a07ba24`, validate/tạo Input Manifest tương ứng; sau đó chạy browser DVH validate→save→replay→refresh→JSON/CSV và CT preview frame/overlay. Giữ P16 closure checklist song song; chỉ chuyển P17 sang staging smoke verified sau khi có run ID, preview hash và direct source evidence.
...
| `technical-specification.md` | 1.11 | Architecture reference; P10/P11/P12 plus P13/P14/P15/P16/P17 bounded-context implementation addenda, resource policy and CT preview adapter contract |
| `plan.md` | 3.5 | Phase/workflow/S-E/C/B tests, DoR/DoD, execution gates, execution ledger, full coverage matrix, P10–P17 checkpoints, binding/report/CT work packages and staging gates |
...
- **Local checks:** CT engine/API focused tests, full backend suite, Ruff, strict mypy, frontend lint/typecheck/Vitest/build, migration and OpenAPI checks passed on the same working tree candidate. Vite still reports the existing bundle-size warning; it is recorded as a performance follow-up, not treated as a functional pass.
- **Not yet evidenced:** deployment of this CT delta on staging, authenticated staging CT artifact upload and browser overlay, direct PostgreSQL row/checksum/scope query, object-storage drift/fault recovery, volume/resource benchmark, full negative matrix, staging P11/P16 actual-limit binding/report source and independent/reference DVH oracle.
...
- **Deployment state:** Railway API/web/worker deployment metadata now shows source SHA `893ae2d46c197b80fc78e29cdea25ab19a54154c` and `SUCCESS` for API `c5807acb-dd49-4279-8304-ecc95141543d`, web `b46d7617-8545-40dc-82b6-d2f42103d696` and worker `3b0b89e3-b633-4477-9bda-48d01af2f3be`. Public OpenAPI contains the CT route and the web bundle contains the CT controls. `/api/v1/version` still reports application build label `9262bfd`, so release metadata must later be made source-identifiable; do not treat the label alone as proof of source parity.
