...
- P20-W01 local support slice: `apps/web/src/api/client.ts` có `ready()` và `PlatformStatusPage.tsx` đọc riêng `/health`, `/ready`, `/version` cùng authenticated `/gamma/queue-metrics`; frontend test readiness failure không cho phép hiển thị nền tảng là ready. Riêng `PlatformStatusPage` local test **2/2**; toàn bộ frontend sau đó là **5 passed**, lint, typecheck và production build PASS; đây chưa phải alert delivery hoặc P20 exit evidence.
