...
- Đây là `STAGING_DEPLOYED_AUTH_E2E_OPEN`, chưa phải `STAGING_VERIFIED`: commit `4d3e0d3` đã lên branch staging; `/api/v1/ready` trả HTTP 200 với schema `20260909_0018`, `/api/v1/openapi.json` có member/invitation routes, và các route invitation/member mới trả HTTP 401 khi chưa xác thực. Effective API/web version vẫn là `65dd52b` do label Railway chưa được cập nhật, nên chưa đạt version parity. Còn phải kiểm Auth thật và hai identity trên browser, xác nhận PostgreSQL rows/hash/status/audit, replay/revoke/expiry/context/scope/concurrency/timeout và cập nhật `APP_VERSION`/`VITE_APP_VERSION` trước khi đóng P04-W02/P04-W04/P04-VERIFY.

## P4 staging deployment delta — 2026-09-09

- **Source/deploy:** commit `4d3e0d3` đã được push lên `codex/p4-org-site-machine`; public staging tự cập nhật. API readiness hiện trả `status=ready`, schema `20260909_0018`; OpenAPI public trả HTTP 200 và chứa các operation `organizations/invitations` và `organizations/{organization_id}/members`.
...
- **Current phase:** P17 — Visual Dose, DVH và structure review; P17 vẫn là phase thực thi chính vì staging case còn thiếu RTSTRUCT/CT và các gate DVH/CT/binding/report/oracle/fault/volume chưa đóng. Song song, P4 invitation/member slice đã có source commit `4d3e0d3` được staging phục vụ với migration `20260909_0018`; không giả định các service cùng effective version khi Railway variables chưa được cập nhật.
- **Current status:** IN_PROGRESS — P17 local slice đã có pure RTDOSE/RTSTRUCT engine, migration `20260908_0017`, API inputs/validate/save/history/export, organization/case scope, checksum/idempotency snapshot, explicit P11/P16 limit binding, DVH report source, bounded CT preview/overlay và route `/app/qa/cases/:caseId/dvh`. P4 public staging hiện trả readiness schema `20260909_0018`, OpenAPI có member/invitation routes, web bundle có UI invitation/member; API/web effective version vẫn `65dd52b` vì `APP_VERSION`/`VITE_APP_VERSION` chưa parity với source candidate. P17 staging saved-run/CT/binding/report, P4 Auth/two-identity/DB lifecycle, P8/P9/P10/P11/P12/P13/P14/P15/P16 closure và production release vẫn mở.
- **Last authoritative check:** 2026-09-09 — full backend suite sau P4 đạt **167 passed**; focused P4 organization **9 passed**, combined P4 health/migration pack **25 passed**, Ruff/mypy, frontend lint/typecheck/Vitest/build và OpenAPI check PASS. Public staging health/readiness/version/openapi/web checks: health/readiness/openapi/web HTTP 200, readiness schema `20260909_0018`, new unauthenticated member/invitation routes HTTP 401; `/api/v1/version` và web bundle còn label `65dd52b` nên config parity chưa đạt. P17 fixture/oracle và authenticated browser evidence lịch sử vẫn giữ ở các mục bên dưới; không dùng chúng để suy ra P4 invitation lifecycle đã staging-verified.
...
| P4 | STAGING CRUD PASS / INVITATION E2E OPEN | Organization/site/machine lifecycle remains evidenced; invitation/member migration `20260909_0018` and public routes are deployed/readiness-ready, but Auth/two-identity browser lifecycle, direct PostgreSQL/hash/status/audit, replay/revoke/expiry/context/concurrency/timeout and version-label parity remain open |
...
- **Local checks:** P17 DVH/CT engine/API/report-source/binding focused suite passed **26/26** (`13` DVH engine, `6` DVH API, `4` report-source, `3` Biological Library); the full backend suite passed **167 tests**, with Ruff, strict mypy, frontend lint/typecheck/Vitest/build, migration and OpenAPI checks passing on the same working tree candidate. Vite still reports the existing bundle-size warning; it is recorded as a performance follow-up, not treated as a functional pass.
...
- **Local evidence:** focused suite `26 passed` (13 DVH engine + 6 DVH API workflows + 4 report tests + 3 Biological Library tests), full backend suite `167 passed`, Ruff and strict mypy pass; frontend lint/typecheck/Vitest pass. Added coverage includes P16 D95 binding, P11 lowercase rule lookup, binding conflict, missing metric, invalid override, fail-closed DVH input discovery/validation when the manifest is pending, invalid, or does not match the artifact checksum, CT Frame of Reference mismatch, no-overlap read-only preview, CT resource guard, and changed CT bytes.
