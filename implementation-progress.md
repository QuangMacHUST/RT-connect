...
- **Last authoritative check:** 2026-09-08 — backend full suite `45/45`, strict mypy/Ruff and frontend typecheck/lint/test/build passed. Staging Gamma runs `9c607a9d-0698-4fc4-bf3f-0b50c3bc474f` and `dd82bdbd-c5af-4929-85fd-c4bea595cadd` completed `PASS` on `gamma-2d-p8.1`; refresh preserved both runs and the compare snapshot. Staging Redis is provisioned and `REDIS_URL` is staged on both API and worker; live verification after the Redis-capable commit is still pending.
- **Next exact step:** push and deploy the Redis queue implementation, verify live queue metrics plus a real worker-consumed Gamma run, then close the explicit failure/retry and RTDOSE/3D gates before starting P9.
...
| P8 | STAGING 2D SYNTHETIC E2E PASS; REDIS DEPLOYMENT VERIFICATION PENDING; FULL EXIT GATE OPEN | Migration `20260907_0007`; organization/case-scoped preflight, idempotent Gamma job snapshot, deterministic 2D engine, Redis-capable worker entrypoint with DB fallback, result map/metrics/warnings, logical-role-aware artifact contract and Gamma Workspace are implemented; staging worker completed two validated JSON runs with PASS, compare and refresh persistence. Live Redis queue/metrics smoke, failure/retry evidence and any retained RTDOSE/3D gate remain |
...
- P8 queue implementation: uncommitted local changes add `redis==6.4.0`, Redis Streams transport (`XREADGROUP`/`XAUTOCLAIM`/`XACK`), API dispatch and retry integration, authenticated `/api/v1/gamma/queue-metrics`, worker Redis mode and focused fake-client tests. Railway staging now has a private `Redis` service and the API/worker service variables contain the private `REDIS_URL` reference; the code deployment and live Redis-consumer smoke are still pending.
