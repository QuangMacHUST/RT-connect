...
- **Local checks:** P17 DVH/CT engine/API/report-source/binding focused suite passed **27/27** (`14` DVH engine, `6` DVH API, `4` report-source, `3` Biological Library); the full backend suite passed **169 tests** on commit `7377aad`, with Ruff, strict mypy, frontend lint/typecheck/Vitest/build, migration and OpenAPI checks passing. The new oracle covers nonuniform slice thickness, volume-weighted Dmean/D(x)/V(x), and the pinned cumulative-DVH interpolation convention. Vite still reports the existing bundle-size warning; it is recorded as a performance follow-up, not treated as a functional pass. Evidence: `docs/evidence/p17-local-volume-weighted-dvh-20260909.json`.
