...
- **Current phase:** P17 — Visual Dose, DVH và structure review; candidate `a546bb1` đã deploy staging đúng schema nhưng browser E2E còn chờ bổ sung RTSTRUCT/CT tổng hợp vào QA case. P16 browser smoke đã pass nhưng database/scope/fault/release closure còn mở, cùng các gate P8/P9/P10/P11/P12/P13/P14/P15 và các phase tích hợp sau.
- **Current status:** IN_PROGRESS — P17 local slice đã có pure RTDOSE/RTSTRUCT engine, migration `20260908_0017`, API inputs/validate/save/history/export, organization/case scope, checksum/idempotency snapshot, explicit P11/P16 limit binding, DVH report source, bounded CT preview/overlay và route `/app/qa/cases/:caseId/dvh`. CT preview local đã decode HU, window/level, chọn frame, map dose/ROI nearest-neighbor trong patient LPS, crosshair, mapping metadata, resource guard và deterministic hash; đây là read-only visual operation, không tạo DVH run. Staging API/web/worker của candidate `a546bb15155baa6ce3b59a86d1f1e1240aaf6c5a` đều `SUCCESS`; `/api/v1/ready` trả `schema_revision=20260908_0017`, `/api/v1/openapi.json` đã expose contract binding mới, web root trả HTTP 200. Staging case hiện có RTDOSE hợp lệ nhưng chưa có RTSTRUCT/CT nên nút tính và CT overlay vẫn chờ input. Staging DVH/CT/binding/report E2E, fault/volume/release evidence chưa đóng. P16/P15 replay/DB/scope gates vẫn mở độc lập.
- **Last authoritative check:** 2026-09-09 — local P17 engine/API/CT focused suite `13 passed`; full backend suite `150 passed`; Ruff/mypy, frontend lint/typecheck/Vitest/build, migration/OpenAPI pass. Railway metadata của deployment trước delta CT xác nhận API deployment `99daa470-cea3-4bd9-a265-6a13632e750d`, web `fa69b146-1f4d-4972-83ec-c14a9bb11a6d`, worker `b9d814d2-dd9d-4caa-9246-30fd377f0ae5`, tất cả `SUCCESS`; staging `/api/v1/health`, `/api/v1/ready`, `/api/v1/version`, `/api/v1/openapi.json` và web root đều trả thành công. Fixture `docs/fixtures/p17-rtstruct-v1-smoke.dcm` được kiểm tra local với `FULL_ROI`, 4 voxels, mean `6.5 Gy`, D95 `5.15 Gy`, SHA-256 `16a79df3129757d9df8b48bd095f0b4b70b24713ea5255e24719d46e6808d401`.
- **Next exact step:** deploy commit chứa CT delta lên staging; sau khi user xác nhận thao tác chọn file, upload fixture RTSTRUCT tổng hợp và một CT fixture hợp lệ vào case `8bc86303-c7e9-4e1a-b012-cfbe2a07ba24`, validate/tạo Input Manifest tương ứng; sau đó chạy browser DVH validate→save→replay→refresh→JSON/CSV và CT preview frame/overlay. Giữ P16 closure checklist song song; chỉ chuyển P17 sang staging smoke verified sau khi có run ID, preview hash và direct source evidence.
...
## P17 CT preview delta — local verified, staging redeploy pending — 2026-09-09
...
- **Deployment state:** this delta is not yet represented by the Railway deployment IDs in the historical P17 deployment section above. After commit/push, API/web/worker must redeploy from the new source SHA and `/api/v1/version`/OpenAPI/browser evidence must be refreshed before calling staging CT preview available.
