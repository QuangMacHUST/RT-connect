...
- **Config parity:** API `/api/v1/version`, web build label và Railway deployment metadata hiện cùng nhận diện `311abed`; staging version parity đã pass. Đây không thay cho release manifest production.
...
- **Last authoritative check:** 2026-09-09 — full backend suite sau P4 đạt **168 passed**; focused P4 organization **9 passed**, combined P4 health/migration pack **25 passed**, Ruff/mypy, frontend lint/typecheck/**5 passed**/build và OpenAPI check PASS. Public staging smoke hiện hành `docs/evidence/p4-staging-public-smoke-20260909.json` đạt **14/14**: health/readiness/version/openapi/web HTTP 200, readiness schema `20260909_0018`, version `311abed`, new unauthenticated member/invitation routes HTTP 401. P17 fixture/oracle và authenticated browser evidence lịch sử vẫn giữ ở các mục bên dưới; không dùng chúng để suy ra P4 invitation lifecycle đã staging-verified.
...
| P4 | STAGING CRUD PASS / INVITATION E2E OPEN | Organization/site/machine lifecycle remains evidenced; invitation/member migration `20260909_0018` and public routes are deployed/readiness-ready on candidate `311abed`, with version-label parity pass; Auth/two-identity browser lifecycle, direct PostgreSQL/hash/status/audit, replay/revoke/expiry/context/concurrency/timeout remain open |
