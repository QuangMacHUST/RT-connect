...
- **Current status:** IN_PROGRESS — P8 has passed the authenticated 2D synthetic staging E2E flow through the deployed Redis Streams worker, including worker completion, result snapshot, compare, refresh persistence, queue metrics and a controlled `FAILED → retry → COMPLETED` recovery. The new local slice adds PSQA/ENGINE_TEST profile enforcement, RTDOSE GY fixture validation, coverage/no-candidate accounting, max-gamma censoring and database-fenced attempt/outbox dispatch. The full P8 exit gate remains open for staging migration, RTDOSE/measurement 3D, independent oracle, crash/ack/retry/resource and large-input evidence.
- **Last authoritative check:** 2026-09-08 — backend full suite `58/58`, strict mypy/Ruff and frontend lint/typecheck/Vitest `1/1`/build passed. The deterministic RTDOSE fixture was regenerated twice with identical SHA-256 `CA5C9168EB9B045E30A375EDC6B76118EFD754A35815C2860B17CA8944C4480B`. Existing staging evidence remains: worker deployment `fb446c0e-2a1b-4a17-88f3-aeb8b5318e3c` from commit `5ec12fb` logged `queue_backend=redis_stream`; run `e084529d-6bb1-4119-ae0a-f4da7d371cac` completed `PASS` on `gamma-2d-p8.1`; controlled run `26a54046-1f20-454c-b3f4-e766937f30d6` recovered from `GAMMA_STORAGE_UNAVAILABLE` at attempt 1 to `PASS` at attempt 2. The new `20260908_0008` slice has not yet been redeployed.
- **Next exact step:** commit/push the P8 slice, deploy migration `20260908_0008` to staging, upload the real synthetic RTDOSE + measurement pair through the browser, execute 3D PSQA through Redis, and record the remaining oracle/failure/resource evidence before starting P9.
...
| P8 | STAGING REDIS STREAM + FAILURE/RETRY PASS; FULL EXIT GATE OPEN | Migrations `20260907_0007` + local unreleased `20260908_0008`; organization/case-scoped preflight, PSQA/ENGINE_TEST profile, deterministic 2D/3D engine with standard-GY RTDOSE adapter, coverage/censor metrics, logical-role-aware artifact contract and database-fenced lease/attempt/outbox slice are implemented and locally tested; prior staging worker completed validated JSON runs with PASS, compare/refresh persistence and controlled storage retry. New slice still needs staging deploy, RTDOSE/measurement 3D, independent oracle, crash/ack/bounded retry/resource/large-input evidence. |
