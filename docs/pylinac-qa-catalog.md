...
| P07-CT | CatPhan | CatPhan 503; 504; 600; 604; 700 của bản phát hành đã khóa | Chuỗi DICOM CT/CBCT hoặc ZIP | HU tuyến tính, hình học, độ dày lát, đồng nhất/NPS, MTF, tương phản thấp theo module có mặt. Cho chọn lát gốc và chỉnh x/y/góc/kích thước ROI/scaling/giá trị HU kỳ vọng. |
...
| P07-LOG | Log Analyzer | Varian Dynalog; Trajectory Log 2.1, 3.0, 4.0; tệp `.bin` và `.txt` khi class hỗ trợ | Đúng hai tệp `.dlg` cho Dynalog; một tệp `.bin`/`.tlog`, có thể kèm `.txt`, cho Trajectory Log | Trục actual/expected/difference, MLC, beam hold, fluence và Gamma log. RT-CONNECT giữ tên tệp A/B, kiểm phiên bản Trajectory và không tự đặt kết luận cuối; người dùng chọn metric/tiêu chí và đánh giá kết quả. |
...
| P07-WL | Winston–Lutz | Winston–Lutz một bi/một trường theo bộ ảnh | Bộ ảnh DICOM/ảnh có scale và góc | BB–CAX từng ảnh, vector/khoảng cách, isocenter/axis plots khi bộ góc đủ, gợi ý dịch bàn nếu dùng. Cho đọc góc từ DICOM, tên tệp hoặc nhập theo thứ tự từng ảnh; khi nhập tay, RT-CONNECT ghép góc vào thành viên ZIP trước khi gọi Pylinac. |
| P07-WLMT | Winston–Lutz Multi-Target | WinstonLutzMultiTargetMultiField và cấu hình multi-target/multi-field công khai | Bộ ảnh nhiều bi/nhiều trường và cấu hình hình học | Ghép cặp field–BB, sai lệch từng target/field, tổng hợp và overlay. UI nhập/điều chỉnh cấu hình BB/field, hệ tọa độ, góc và mapping; bảng kết quả theo từng ảnh/bi lấy trực tiếp từ `image_details`; không bắt người dùng sửa tệp cấu hình thô. |
