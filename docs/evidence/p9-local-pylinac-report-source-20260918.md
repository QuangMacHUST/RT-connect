...
- Lớp chụp nguồn giữ `overlay_artifact_id`; khi người dùng chọn phần hình, trình dựng PDF hoặc PNG lấy đúng tệp dẫn xuất trong cùng đơn vị, giải mã ảnh an toàn và nhúng vào phần hình phân tích. Nếu thiếu ảnh, tệp văn bản vẫn được xuất kèm cảnh báo rõ ràng.
- Kiểm thử tích hợp tạo lượt hiệu chuẩn Pylinac tổng hợp có ảnh, dùng lượt đó làm nguồn báo cáo, xuất PDF và PNG, rồi xác nhận snapshot/lớp phủ được giữ nguyên: **10/10** kiểm thử `tests/test_reports.py` đạt.
- Kiểm tra Ruff và mypy cho API báo cáo đạt; toàn bộ giao diện đạt **16/16 tệp và 80/80 phép thử**.

## Giới hạn còn mở

Lát cắt này chưa kiểm chứng phiên có đăng nhập trên staging và chưa đối chiếu xem trước với tài liệu xuất. P9 vẫn mở; kết quả này không phải bằng chứng sẵn sàng lâm sàng.
