...
- Lớp chụp nguồn giữ `overlay_artifact_id` để lát cắt renderer tiếp theo có thể lấy đúng hình/lớp phủ theo cùng đơn vị. Lát cắt này chưa nhúng ảnh vào PDF.
- Kiểm thử tích hợp tạo lượt hiệu chuẩn Pylinac tổng hợp, dùng lượt đó làm nguồn báo cáo và xác nhận snapshot kết quả được giữ nguyên: **10/10** kiểm thử `tests/test_reports.py` đạt.
- Kiểm tra Ruff và mypy cho API báo cáo đạt; toàn bộ giao diện đạt **16/16 tệp và 80/80 phép thử**.

## Giới hạn còn mở

Lát cắt này chưa kiểm chứng phiên có đăng nhập trên staging, chưa nhúng lớp phủ vào PDF/PNG và chưa đối chiếu xem trước với tài liệu xuất. P9 vẫn mở; kết quả này không phải bằng chứng sẵn sàng lâm sàng.
