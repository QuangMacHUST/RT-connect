...
> P0/P18 backend regression recheck ngày 2026-09-11 trên SHA `249a8badb042eb903c8cb5169f1f80fa35dcf172`: collection `204 test/31 file`; ba partition độc lập đạt `44/44`, `59/59`, `101/101` với exit code `0`, tổng **204/204 PASS**. Lệnh monolithic bị dừng không có summary/exit code nên không tính PASS và được ghi rõ trong evidence `docs/evidence/p0-p18-backend-regression-20260911-249a8ba.json`. Đây là `LOCAL_VERIFIED`; các gate remote vẫn mở.

> P18-W00 local integrated release-candidate recheck ngày 2026-09-11 trên SHA `4ecfae863991cf32138e7c47c38558b2950e9bf6`: `tests/test_p18_integration.py` đạt **2/2 PASS**. QA journey đi qua auth/session, nested folder/case, Machine QA, artifact validation, Gamma queue/idempotency/worker, report export/download hash và trend rebuild; Biological journey đi qua library, BED/EQD2, comparison, re-irradiation và report độc lập không có `qa_case_id`. Evidence: `docs/evidence/p18-local-integrated-20260911-4ecfae8.json`. Đây chỉ là `LOCAL_VERIFIED` support evidence; P18 staging E2E, fault/resource, provider restore, pilot và P19/P20 release gates vẫn mở.
...
> P18/P19/P20 provider backup inspection ngày 2026-09-11: Railway GraphQL read-only introspection xác nhận có các surface `volumeInstanceBackupList`, `volumeInstanceBackupScheduleList` và `volumeInstancePitrRestoreEstimate`; volume PostgreSQL staging (`a3380835-08d5-409a-8d46-cd15931445b4`) và production (`56d3e3cf-8540-44f1-b2eb-8fc53fbd590f`) đều `READY` nhưng trả `0` backup và `0` schedule. Không gọi mutation. Evidence: `docs/evidence/p18-p20-railway-backup-capability-20260911.json`. Trạng thái chính thức: `PROVIDER_BACKUP_NOT_CONFIGURED`; P18 backup/restore, P19 promotion và P20 restore drill vẫn mở.

> P20 backup authority check ngày 2026-09-11: mutation staging `volumeInstanceBackupScheduleUpdate(kinds=[DAILY])` bị Railway trả `Not Authorized` với cả account-scoped và project-scoped token; read-back xác nhận state không đổi (`0` schedule, `0` backup). Evidence: `docs/evidence/p20-railway-backup-schedule-authority-20260911.json`. Đây là `EXTERNAL_PERMISSION_REQUIRED`, không phải lý do để đánh dấu `NOT_APPLICABLE`; cần cấp quyền hoặc cấu hình qua dashboard rồi tiếp tục read-back/restore drill.
...
> Local update P8 workload ngày 2026-09-11: script `scripts/benchmark-p8-gamma.py` chạy engine `gamma-nd-p8.2` trên hai grid 3D tổng hợp giống nhau `8×16×16` (2,048 voxel), 2 lần lặp, GRID/3D, DTA 3 mm, max γ 2 và candidate budget 50,000. Kết quả median `3.183637 s`, max `3.307343 s`, Python-traced peak tối đa `2,023,628 bytes`; oracle `PASS`, 2,048/2,048 evaluated/passing, pass rate `100%`, max gamma `0`, không censor/no-candidate. Evidence: `docs/evidence/p8-local-gamma-workload-20260911-4cf0164.json`. Đây là local deterministic workload/oracle evidence, không phải Railway capacity, staging large-input hoặc clinical commissioning; các gate đó vẫn mở.
...
- Kiến trúc tham chiếu: [technical-specification.md](technical-specification.md) v1.29.
...
> Operational update P18 API restart recovery ngày 2026-09-10: API staging được yêu cầu redeploy có kiểm soát cùng SHA `135a24faf4e2659b8c4fe9a98920efb357b392bc`, không xóa database/object. Probe đầu tiên sau yêu cầu trả `ready`, schema `20260909_0019`, version đúng SHA; public verifier sau propagation đạt `15/15`, browser reload Gamma giữ API thật, fixture và preflight `VALID`. Evidence: `docs/evidence/p18-staging-api-restart-recovery-20260910-135a24f.json`. Đây là `STAGING_PARTIAL_API_RESTART_RECOVERY`, không thay cho fault injection API trong lúc có job, DB/Redis/storage/renderer failure, restore hoặc rollback.

> Operational update P19-W01 release manifest ngày 2026-09-10: manifest redacted cho candidate `staging-9f8378c` đã dùng deployment ID thật của API `db38336d-649e-4e05-baa8-2c669d486c2b`, web `d00e2ac1-d16b-41a6-88ae-48f9185a7608` và worker `07068c3e-15af-439f-a837-e565e90b2f84`; cả ba cùng SHA `9f8378c171f8b220f3c5b4159f505479bcd662c0`, schema `20260909_0019`, fixture hash và engine version được kiểm. `--verify-manifest` trả `valid=true`, `error_count=0`, `service_sha_parity=true`; production test trong manifest vẫn ghi `PRODUCTION_PROMOTION_BLOCKED_PROVIDER_BACKUP_RESTORE_ROLLBACK`. Evidence: `docs/evidence/release-manifest-staging-9f8378c.json`. `release_gate=ELIGIBLE` ở đây chỉ là integrity/source-parity gate của manifest staging, không đóng P18/P19 và không cho phép promote production.
...
> Operational update P8/P18 local queue ngày 2026-09-10: sau khi Redis local trong `docker-compose.yml` được khởi động lại, `scripts/verify-local-gamma-queue.py` chạy trên commit `3fe1db2ab3139dec97090d0660278e419c8c6177` đạt `passed=true`. Evidence `docs/evidence/p8-local-redis-worker-smoke-20260910.json` ghi happy path, duplicate dispatch/terminal replay, injected ACK failure với pending→0 và không tạo attempt thứ hai, bounded retry/dead-letter sau 3 output, malformed-message quarantine không sao chép payload, cùng cleanup stream/key/bucket/database tạm. Đây là `LOCAL_COMPOSE_REDIS_WORKER` support evidence; không đóng staging fault injection, capacity/resource, provider restore hoặc production gate.
...
> Operational update P18 follow-up ngày 2026-09-10: Docker Desktop/Linux engine đã được khôi phục mà không xóa container, volume hoặc image. Chạy lại `scripts/verify-local-backup-restore.py --command-timeout-seconds 30` đạt `passed=true`: PostgreSQL custom dump `169,924` bytes, source/restored row inventory khớp, source/restored object inventory cùng `0` object và cùng SHA-256 `44136fa355b3678a1146ad16f7e8649e94fb4fc21fe77e8310c060f61caaff8a`, disposable database/bucket đã cleanup. Evidence mới: `docs/evidence/p18-local-backup-restore-20260910.json`; evidence timeout trước đó vẫn được giữ để chứng minh bounded failure path. Đây chỉ là `LOCAL_SUPPORT_ONLY`, chưa đóng provider backup/restore, RPO/RTO staging/production hoặc P18/P20.

> Operational update P19 parity ngày 2026-09-10: commit `8dffbb9f48906131e99143f11b14d2abecd9a233` đã được yêu cầu deploy đồng thời cho API, web và worker staging. Public verifier exact-SHA đạt `15/15`, `failed_check_count=0`, API `/health`/`/ready`/`/version`, schema `20260909_0019`, OpenAPI membership/invitation, unauthenticated `401` boundary, web index/bundle và source marker đều PASS. Evidence: `docs/evidence/p19-staging-public-smoke-20260910-8dffbb9.json`. Đây chỉ đóng public source/runtime parity sub-gate; authenticated E2E đầy đủ, direct dependency/database evidence, fault/resource, backup/restore, rollback, alert và production promotion vẫn mở.

> Operational update P19 effective-settings/readback ngày 2026-09-10: Railway GraphQL read-only verifier đã kiểm tra đúng project/environment và ba service staging. API dùng `/apps/api`, `/apps/api/Dockerfile`, health `/api/v1/health`, pre-deploy duy nhất `alembic upgrade head`; worker dùng `/apps/api`, `python -m rt_connect_api.worker`, không healthcheck/pre-deploy và không có public-domain variable; web dùng `/apps/web`, `/apps/web/Dockerfile`, không healthcheck/pre-deploy. Required Auth/DB/Redis/S3/API variables có mặt, token/service-role/browser database names bị loại, DB/Redis được phân loại private Railway host, S3 được phân loại external S3-compatible endpoint, CORS không wildcard và chứa đúng staging web origin, `VITE_API_BASE_URL` khớp API HTTPS. Kết quả `passed=true`, `failed_check_count=0`, không ghi secret/database URL: `docs/evidence/p19-railway-effective-settings-20260910.json`. Browser authenticated read-only trên candidate `3e79f488...` đã đọc organization, QA archive, DVH, report, Biological và system status; deep-link/reload DVH giữ run `d8230d1d...` và D95 `5.200 Gy`: `docs/evidence/p19-staging-authenticated-remote-readback-20260910-3e79f48.json`. Đây vẫn là partial P19; two-identity Auth, mutation E2E, export/download, fault/restore, rollback và production promotion còn mở.

> Operational update P18 worker-restart recovery ngày 2026-09-10: trên staging, worker được redeploy cùng commit `1c0210c4286e7aeb3dbb565ce34e5ab474dfa708` để mô phỏng restart có kiểm soát; deployment `b007ed76-f3c6-4ba8-b70d-9001503ad9db` báo `SUCCESS`. Trước/sau, status page giữ health `OK`, readiness `READY`, schema `20260909_0019`, Redis `available/configured`, stream `9`, pending `0`; không xóa DB/object và không tạo run mới. Evidence: `docs/evidence/p18-staging-worker-restart-recovery-20260910-1c0210c.json`. Đây chỉ là `STAGING_PARTIAL_RESTART_RECOVERY`, chưa đóng P18-W02 fault matrix.
...
> Operational update P17/P19 candidate parity ngày 2026-09-10: sau khi push commit tài liệu `e5197ce4634f93b81267479d9b68382e97f7e4e9`, web staging tự nhận SHA mới trong khi API/worker còn `380012e`; verifier exact-SHA với expected `380012e` đã fail đúng một check `web.bundle.expected_version`, evidence drift được giữ tại `docs/evidence/p19-staging-public-recheck-20260910.json`. Đã redeploy có kiểm soát cả API, web và worker lên cùng `e5197ce`; deployment thật là API `d3e6c135-6602-420e-9609-d4ba9098e5fe`, web `b670b3de-cecc-473c-960b-58a7374fb1da`, worker `675a146e-cace-4c63-bf28-fdf7eca25bef`, đều `SUCCESS`. Public verifier sau đồng bộ đạt `15/15`, schema `20260909_0019`, source parity PASS tại `docs/evidence/p19-staging-public-recheck-20260910-e5197ce.json`. Browser authenticated đọc lại case `8bc86303-c7e9-4e1a-b012-cfbe2a07ba24` và fixture RTDOSE `gamma-rtdose-v1-smoke.dcm` `VALID`, SHA `ca5c9168eb9b045e30a375edc6b76118efd754a35815c2860b17ca8944c4480b`, saved DVH run/result không đổi, không upload/artifact/run mới; evidence `docs/evidence/p17-staging-rtdose-authenticated-recheck-20260910-e5197ce.json`. Không xóa database/object. Đây chỉ là candidate parity + authenticated readback; không đóng các gate full E2E, fault/resource, backup/restore, rollback, alert, production hoặc clinical readiness.

> Current candidate checkpoint P17/P19 ngày 2026-09-10: commit `19857bd8515bfbc86835527aadc145278b837dc3` đã được deploy đồng thời cho API, web và worker staging; deployment IDs API `60c4bcde-2ddc-4b6a-b586-d51385926c1c`, web `7cd043f4-854d-4399-bdb3-dd2ff0639003`, worker `74c69765-7440-4953-9e30-21338b31fe2d`, đều `SUCCESS`. Public verifier exact-SHA `19857bd` đạt `15/15`, schema `20260909_0019`, tại `docs/evidence/p19-staging-public-recheck-20260910-19857bd.json`. Browser authenticated reload trên đúng build đọc lại case `8bc86303-c7e9-4e1a-b012-cfbe2a07ba24`, RTDOSE `gamma-rtdose-v1-smoke.dcm` `VALID`, SHA `ca5c9168eb9b045e30a375edc6b76118efd754a35815c2860b17ca8944c4480b`, RTSTRUCT/ROI hợp lệ và saved DVH run/result giữ nguyên; không tạo mutation và không upload trùng, evidence `docs/evidence/p17-staging-rtdose-authenticated-recheck-20260910-19857bd.json`. Planning contract sau khi đồng bộ verifier expected versions đạt `passed=true`, `failed_check_count=0`, đủ 21 phase tại `docs/evidence/p0-planning-contract-20260910-e5197ce.json`. Đây chỉ là parity/readback partial, không đóng full P17/P18/P19/P20, rollback, provider restore, alert, pilot hoặc production readiness.
...
| P8 | 2D JSON/Redis worker/retry và 3D PSQA RTDOSE + measurement đã có staging evidence; browser recheck trên `ec50990` thấy `PREFLIGHT VALID`, run 3D `8/8 PASS`; local independent oracle 6/6 case PASS; local Compose worker smoke đã chứng minh dispatch/replay/bounded retry/dead-letter | Semantic/scientific oracle promotion, staging lease/outbox crash/ack failure injection, staging resource limits và large workload chưa được coi là hoàn tất. |
| P9 | Có implementation slice backend/frontend và local tests cho template, revision, block, snapshot, renderer, export và idempotency; authenticated browser smoke đã tạo revision và tải JSON/CSV/PDF/PNG trên staging. Candidate hiện hành `bd4792505acea66113953f1d525e571cbe8ca155` đã đồng bộ API/web/worker và content-level export verification đạt. | Storage failure/retry, filename rename độc lập trong browser, visual human review và release manifest vẫn là gate riêng. |
| P10 | Có implementation slice backend/frontend, migration `20260908_0010`, test scope/error/rebuild/event/export và trend UI; API đã có count-preflight và raw/aggregate budget | Local contract test đã chứng minh raw vượt budget bị chặn trước materialize, aggregate trong budget trả bucket + lineage và CSV aggregate có record `BUCKET`; staging large-series benchmark, complete negative matrix, source/export revalidation và visual/accessibility evidence còn phải làm. |
...
| P17 | Có engine/API/UI/migration/test local; explicit P11/P16 binding, DVH report source, bounded CT renderer/overlay, local 1M-voxel volume measurement, local Docker 2-job concurrent measurement và local `LOCAL_RESOURCE_GATE_PASS` đã có evidence; staging case đã có RTDOSE/RTSTRUCT/CT và authenticated browser đã chạy saved-run/replay/refresh, CT overlay/no-overlap smoke, export-content probe, targeted PostgreSQL row/checksum/scope probe và object-storage byte re-hash. Browser recheck hiện quan sát cùng run/snapshot trên candidate staging mới nhất | Guard ORM + migration PostgreSQL `20260909_0019` đã deploy; schema/readiness/source SHA public smoke trên candidate `a5c554cb15a37a9dc4ae3e4ff65ab003c20c4763`, trigger metadata và transaction-local negative probe PASS, không chạm run retained. Export payload JSON/CSV parse/hash PASS nhưng browser final filename vẫn UNVERIFIED; còn binding/report cloud, negative/fault/volume, independent-oracle promotion và release manifest. |
...
| GAP-09: Build label không chứng minh source deployed | API và frontend đã có contract ưu tiên `RAILWAY_GIT_COMMIT_SHA`; Git-triggered Docker build khai báo SHA làm build argument, còn `APP_VERSION`/`VITE_APP_VERSION` là fallback local/manual. Release manifest vẫn là lớp bắt buộc cho promotion production. | P3/P19 | Deploy lại candidate và chứng minh API/web/worker runtime label cùng source SHA, sau đó ghi SHA từng service vào manifest redacted. | `STAGING_VERIFIED` cho API/web source SHA trên candidate `62a7b5300e39b9e742bee0b7353ff86a559d4abe`; worker và production release manifest vẫn `OPEN`. |

Các trạng thái trên chỉ là checkpoint, không phải đóng phase. `LOCAL_VERIFIED` nghĩa là có code và test local tương ứng; chỉ `STAGING_VERIFIED` mới chứng minh luồng browser → API → database/object storage → worker → result trên release đang chạy. GAP-06, phần worker/production manifest của GAP-09 và phần staging/oracle/benchmark của P8 vẫn chặn DONE-v2; riêng source-label API/web staging của GAP-09 đã được xác minh.
...
- Frontend trên candidate hiện tại: lint, typecheck, Vitest **11 passed** (trong đó `SessionErrorPage` onboarding **3/3**, Archive DVH preflight **3/3** và Platform Status/readiness **3/3**) và production build PASS; build chỉ còn cảnh báo bundle JavaScript >500 kB, không phải lỗi functional. Evidence: `docs/evidence/p3-p4-frontend-onboarding-20260909.json`, `docs/evidence/p19-staging-source-label-20260909.json` và test source `apps/web/src/pages/QAArchivePage.test.tsx`/`apps/web/src/pages/PlatformStatusPage.test.tsx`. Các dòng `1/1` ở những checkpoint lịch sử bên dưới được giữ nguyên theo candidate cũ và không đại diện cho tổng test hiện tại.
...
- P10 trend slice: migration `20260908_0010` đã upgrade thành công trên local PostgreSQL; `TrendPoint` có context snapshot, uniqueness theo organization/source run/metric và index theo thời gian. Baseline version, maintenance event/revision, raw/day/week query, compatibility signature, outlier, rebuild idempotency, export và source drill-down đã có API/UI; P10 test riêng **8/8 PASS**. Slice hiện hành bổ sung count-preflight, raw/aggregate budget và bucket export lineage; staging large-series benchmark và complete S/E/C vẫn mở.
...
- P19 source-label recheck: candidate `62a7b5300e39b9e742bee0b7353ff86a559d4abe` đã được Railway staging deploy. API `/api/v1/health` và `/api/v1/ready` trả HTTP 200 với `status=ok/ready`, schema `20260909_0018`; `/api/v1/version` trả đúng source SHA. Web root và bundle mới trả HTTP 200, bundle chứa source SHA; browser `/app/system/status` hiển thị cùng SHA, `SẴN SÀNG`, `OK`, `READY`, `MATCH` và Redis queue `available/configured`. Evidence: `docs/evidence/p19-staging-source-label-20260909.json`. Đây chỉ đóng source-label staging sub-gate; worker parity, production manifest, rollback và clinical release gates vẫn mở.
...
- **Checkpoint staging mới nhất trước source-label fix (2026-09-09):** candidate `2fcf065` đã được deploy trên API/web/worker staging; API `/api/v1/ready` trả `200` với schema `20260909_0018`, `/api/v1/version` trả version `2fcf065`, OpenAPI đã có member/invitation routes, và các route mới không có Auth trả `401`. Public evidence `docs/evidence/p4-staging-public-smoke-20260909-2fcf065.json` đạt **14/14** và version parity đã pass. Auth browser với hai identity, PostgreSQL row/hash/status/audit, replay/revoke/expiry/context/concurrency/timeout vẫn mở.

- **Source-label recheck staging (2026-09-09):** candidate `62a7b5300e39b9e742bee0b7353ff86a559d4abe` đã được deploy; API `/health`, `/ready`, `/version` và browser `/app/system/status` cùng nhận diện source SHA, schema `20260909_0018`, trạng thái `SẴN SÀNG/READY/MATCH`, còn queue hiển thị `available/configured`. Evidence `docs/evidence/p19-staging-source-label-20260909.json`. Đây chỉ đóng source-label API/web sub-gate; worker parity, Auth browser với hai identity, PostgreSQL row/hash/status/audit, replay/revoke/expiry/context/concurrency/timeout vẫn mở.
...
### Checkpoint local Redis/worker — 2026-09-10
...
### Checkpoint staging P9/P19 — current exact-SHA report export content — 2026-09-10 / `bd47925`
...
- [x] P10-W04 — Baseline version/event CRUD; chart keyboard/table fallback/export metadata. `LOCAL_VERIFIED` cho API/UI và test; staging đã kiểm authenticated update/archive, stale-conflict readback và không rewrite trend history trên candidate `734318947b8ae51c9eebc5275e76e6bdc6478d18`. Visual/accessibility, complete S/E/C, large-series, effective-time/rebuild, provider fault/resource và release evidence vẫn mở.
...
| TC-P10-S04 | Dữ liệu lớn | Raw vượt budget bị trả 413 trước khi materialize; day/week trong aggregate budget có cảnh báo large-query, min/max/count và source IDs/run IDs; raw drill-down theo filter hẹp vẫn truy cập được. |
...
| TC-P10-E11 | Raw query vượt giới hạn, aggregate vượt giới hạn hoặc export dependency fail | 413/503; raw được gợi ý chuyển day/week, aggregate quá lớn được gợi ý thu hẹp filter, lỗi export retry an toàn; không trả CSV/JSON bị cắt hoặc CSV aggregate rỗng như success. |
...
- **Trạng thái hiện tại:** `STAGING_DVH_CT_BROWSER_DIRECT_DATA_PROBES_PASS_GATES_OPEN`; engine/API/UI/migration/test local đã có, explicit P11/P16 limit binding, DVH report source và bounded CT renderer đã có code/test local. Migration P17 `20260908_0017` và guard bất biến `20260909_0019` đã được triển khai; public smoke mới nhất trên candidate `a5c554cb15a37a9dc4ae3e4ff65ab003c20c4763` đạt 15/15 checks với schema `20260909_0019`, còn trigger `trg_dvh_analysis_runs_immutable` và transaction-local negative probe được ghi ở evidence triển khai trước đó, không chạm run retained. Case smoke `8bc86303-c7e9-4e1a-b012-cfbe2a07ba24` hiện có RTDOSE GY hợp lệ, RTSTRUCT và CT hợp lệ; browser recheck đã xác nhận lại preflight, ROI tự nạp, DVH saved-run/replay/refresh và CT frame `#1` overlay. Read-only PostgreSQL query trong API container đã xác nhận 3 artifact rows, 3 input manifests, 1 DVH run, scope organization/case và manifest-to-artifact checksum; object-storage byte re-hash và snapshot-field recheck cũng PASS. Trên candidate hiện hành `c11fca0451f0a16d3bdf178383d7a8f45889e770`, JSON export `24,673` bytes và CSV export `15,901` bytes đã parse thành công, với run/result fingerprint khớp provenance và CSV đủ section `input/result/warning`; evidence `docs/evidence/p17-p19-staging-dvh-export-current-20260910-c11fca.json`. `.crdownload` chỉ còn là giới hạn quan sát tên file của CUA harness, không phải payload thiếu; filename rename độc lập vẫn chưa được tuyên bố. Binding/report cloud, fault/resource/volume, full negative matrix, independent-oracle promotion và release evidence chưa đóng.
- **Report-source staging addendum:** trên browser build `da963343e1b691c0d23358c2f4520b0d84f9c2e4`, Report Builder đã tạo report source type `DVH`, revision 1, từ đúng run `8000ff9b-7a02-4cec-850e-e27e4fe50cc4`; snapshot SHA `fd99a61aed10191ae169155bfd645c02ef56ffda2ded03384efe43b22d01ea85`, source fingerprint/result SHA và artifact checksum snapshot khớp run. JSON export job báo `10,536` bytes và object-storage document mở được, với `source_id`/`content_sha256` khớp revision; không tạo lại DVH và không sửa run. Vì run này chưa có limit binding, gate còn lại là P11/P16 binding staging, không gộp report-source PASS vào binding PASS.
...
- [x] P17-W05b.2a — ORM/database immutability implementation: `DVHAnalysisRun` mapper guard cho update/delete, migration `20260909_0019` với PostgreSQL trigger và SQLite local trigger tương đương; focused negative mutation test PASS.
- [ ] P17-W05b.2 — Staging CT artifact upload/validation, authenticated browser overlay, warning/error matrix và direct source/hash evidence; partial staging smoke đã đạt S12–S16, no-overlap warning, export-content parse/hash, targeted PostgreSQL row/checksum/scope probe, object-storage byte re-hash, snapshot-field recheck và guard `20260909_0019`/negative mutation probe với fixture tạm. Public/browser source parity đã được recheck trên `a5c554c`; trên candidate `c11fca0` JSON/CSV export content đã parse/hash đúng snapshot và evidence browser tương đương đã được ghi tại `p17-p19-staging-dvh-export-current-20260910-c11fca.json`; `.crdownload` filename rename vẫn không được tuyên bố. Chỉ đóng khi hoàn tất E24–E30, các assertion dữ liệu còn lại, fault/resource/volume và release evidence.
- [ ] P17-W06 — Large workload/async execution hoặc explicit resource policy, storage/dependency fault injection, volume benchmark và staging browser E2E. `LOCAL_VOLUME_MEASURED`: synthetic 64×128×128 (`1,048,576` voxel) chạy engine `p17-dvh-1.1.0`, benchmark runner đã được sửa để chạy độc lập từ repository root; lần đo mới nhất có median host `1.0823655 s`, min `1.0765329 s`, max `1.0983356 s`, peak Python-traced allocation `69,235,550 bytes`, oracle `Dmean=2.5 Gy`, `Dmin=1 Gy`, `Dmax=4 Gy`, selected volume `12,582.912 cc`; RSS host Windows không đo được nên `performance_gate=NOT_ASSESSED`. Evidence mới: `docs/evidence/p17-local-volume-benchmark-20260911-183a4d2.json`; evidence lịch sử host/Docker giữ nguyên tại `docs/evidence/p17-local-volume-benchmark-20260909.json` và `docs/evidence/p17-local-docker-volume-benchmark-20260909.json`. `LOCAL_RESOURCE_GATE_PASS`: `docker-compose.yml` pin API local `1 CPU/768 MiB`; verifier `scripts/verify-p17-docker-workload.ps1` chạy 2 job đồng thời, mỗi job 3 lần trên cùng workload; cả 6 lần giữ engine `p17-dvh-1.1.0`, oracle `Dmean=2.5 Gy`, `Dmin=1 Gy`, `Dmax=4 Gy`, selected volume `12,582.912 cc`; process peak RSS lớn nhất `127,086,592 bytes`, elapsed tối đa `2.0811 s`, p95 `/health` `166.016 ms`, resource budget và API responsiveness đều PASS, schema `20260909_0019`; cgroup v1 peak `385,990,656 bytes` kể từ lúc container start được lưu nhưng không dùng làm peak RSS. Evidence: `docs/evidence/p17-local-docker-workload-20260910.json`. Đây mới là local resource gate; fault injection, worker/service capacity Railway, staging workload và P8 Gamma large-workload vẫn mở.
...
- [x] P17-VERIFY-LOCAL — `test_dvh_engine.py` **14 passed**, `test_dvh.py` **7 passed**, `test_reports.py` **4 passed**, `test_biological_library.py` **3 passed** (focused P17/P16/report/library **28 passed**); full backend **169 passed**, Ruff, strict mypy, frontend lint/typecheck/Vitest/build và migration/OpenAPI checks đã pass với engine commit `cdf4372` (`p17-dvh-1.1.0`), được kiểm lại trên clean verification commit `50d890e`. Các case mới bao phủ explicit P11/P16 binding, missing/invalid/mismatched manifest checksum, CT Frame of Reference mismatch, CT no-overlap read-only preview, CT resource guard, changed CT bytes, saved snapshot detail/export stability after source-byte drift, Compose schema-revision type regression và volume-weighted DVH với nonuniform slice thickness. D95 known-answer được pin ở `5.20 Gy` theo cumulative-DVH convention. Benchmark local P17-W06 đo synthetic 1,048,576 voxel trên host và API container sau rebuild đúng source; verifier workload Docker đồng thời đạt 2 job × 3 lần, không có patient data và giữ cùng oracle; evidence: `docs/evidence/p17-local-volume-weighted-dvh-20260909.json`, `docs/evidence/p17-local-volume-benchmark-20260909.json`, `docs/evidence/p17-local-docker-volume-benchmark-20260909.json` và `docs/evidence/p17-local-docker-workload-20260909.json`. Đây chưa phải controlled RSS/service-concurrency/staging performance gate.
- [ ] P17-VERIFY-STAGING — migration P17 `20260908_0017` và guard `20260909_0019` trong schema head staging `20260909_0019`, readiness/schema/source parity, authenticated browser input→validate→save→refresh→export, direct PostgreSQL row/checksum/scope, negative/fault/volume evidence. Browser input→validate→save→refresh, CT frame `#1/#3`, export-content parse/hash trên candidate `c11fca0`, targeted direct PostgreSQL row/checksum/scope, object-storage byte re-hash, snapshot-field recheck, public 15/15 smoke và guard probe đã được xác nhận; fault/resource/volume, full negative matrix, binding/report/release evidence còn mở. `.crdownload` content-level completion đã có evidence tương đương, không ghi nhận filename rename độc lập.
...
| TC-P17-S18 | Sau khi một run đã commit, thử sửa/xóa qua ORM và trên disposable database thử `UPDATE/DELETE` trực tiếp | ORM trả `DVH_RUN_IMMUTABLE`; PostgreSQL/SQLite trigger từ chối mutation; row, timestamps, input/result hash và export vẫn giữ nguyên; thay đổi phải đi qua run mới. |
...
| TC-P17-E31 | Code hoặc thao tác vận hành cố `UPDATE`/`DELETE` một `DVHAnalysisRun` đã lưu, kể cả chỉ sửa `updated_at` | `DVH_RUN_IMMUTABLE` | ORM/database từ chối transaction; query lại để xác nhận snapshot không đổi; không retry mutation, không xóa lịch sử; tạo run mới với input/config/idempotency key mới nếu cần tính lại. |
...
- **Tính bất biến:** raw DICOM không sửa; `DVHAnalysisRun` là snapshot đọc lại được và append-only; ORM/database chặn update/delete với `DVH_RUN_IMMUTABLE`; source đổi sau đó không làm thay đổi run cũ; replay cùng key/fingerprint trả cùng run. Thay đổi input/config phải tạo idempotency key và run mới.
...
- **Staging exit:** đúng candidate với migration P17 `20260908_0017` và guard `20260909_0019` nằm trong schema head; browser→API→Railway PostgreSQL/object storage; trigger tồn tại và negative mutation probe dùng fixture được cho phép, không sửa/xóa run retained; direct DB row/hash/scope và object-storage byte re-hash; positive/negative/idempotency/reconnect/export; fault injection và workload trong budget.
...
- **Direct-data follow-up hiện hành:** sau browser DVH smoke, read-only PostgreSQL probe vẫn giữ `artifact_count=3`, `manifest_count=3`, `run_count=1` và scope/checksum assertions PASS. Read-only object-storage probe tải lại đúng ba object của case, quan sát kích thước `898/866/842` bytes và re-hash khớp SHA-256 artifact; assertion `object_count=3`, `byte_rehash=True` PASS. Evidence đầy đủ ở `docs/evidence/p17-staging-dvh-ct-browser-20260909.json`; immutable-snapshot mutation guard, full fault/resource/volume, binding/report cloud và release vẫn mở.
...
### Checkpoint public staging sau migration P17 guard — 2026-09-09

Candidate `d04fb5a3561804bd3553b36886902e1c50b2f18f` đã được API/web staging phục vụ. `scripts/verify-public-deployment.ps1` với expected source SHA và schema `20260909_0019` đạt **15/15 checks PASS**: health/readiness, schema/version parity, OpenAPI CT preview + membership/invitation, unauthenticated boundary 401, web index/bundle, CT/membership UI marker và bundle source SHA. Evidence: `docs/evidence/p19-staging-public-smoke-20260909-d04fb5a.json`. Đây chỉ là public parity sub-gate; không thay Authenticated E2E, trigger/database probe, worker parity, remote workflow, backup/rollback hoặc production manifest.

### Checkpoint public staging và browser recheck sau source-parity commit — 2026-09-09

Commit `7a340b281f0bf8a91a5c72dc861c71c82009bce8` đã được API/web/worker staging phục vụ để làm rõ source parity giữa các service. `scripts/verify-public-deployment.ps1` đạt **15/15 checks PASS** với schema head `20260909_0019`: health/readiness, `/version`, OpenAPI, unauthenticated membership boundary, web bundle và source marker đều khớp. Evidence: `docs/evidence/p19-staging-public-smoke-20260909-7a340b2.json`. Browser `/app/qa/cases/8bc86303-c7e9-4e1a-b012-cfbe2a07ba24/dvh` được reload trên build `7a340b2`; preflight `2 RTDOSE · 1 RTSTRUCT · 1 CT`, saved run `8000ff9b-7a02-4cec-850e-e27e4fe50cc4`, ROI `#1 · P17_TARGET` và CT overlay `LPS LINKED` được quan sát lại. Evidence tổng hợp: `docs/evidence/p17-staging-dvh-ct-browser-20260909.json`.

JSON/CSV export hiện đã được capture, parse và hash đúng snapshot; tuy nhiên Edge/CUA giữ suffix `.crdownload`, nên việc đổi sang filename cuối vẫn là `UNVERIFIED`. Đây là open observation của browser/runtime harness, không được ghi thành PASS giả và không làm mất bằng chứng payload. Checkpoint này đóng public source/schema parity sub-gate và browser content recheck, nhưng không thay Authenticated E2E đầy đủ, trigger/database probe, worker parity, fault/resource/volume, report/binding, backup/rollback hoặc production manifest.

### Checkpoint source-parity recovery — 2026-09-09

- Root cause đã được xác định: commit chỉ thay đổi tài liệu ở thư mục gốc làm web staging tự build, trong khi path filter của API bỏ qua commit đó; vì vậy API vẫn trả source SHA cũ dù cả hai service đều Online. Đây là lỗi release/source-parity, không phải lỗi fixture, database hay DVH.
- Đã đặt parity marker trong cả `apps/api/README.md` và `apps/web/README.md`, push commit `b0263c932c740d4f36f19867241d5c1e07014765`, chờ Railway deploy đồng bộ API/web/worker rồi chạy lại `scripts/verify-public-deployment.ps1` với schema `20260909_0019`.
- Kết quả **15/15 checks PASS**: health/readiness/version, schema head, OpenAPI CT + membership/invitation, unauthenticated 401 boundary, web index/bundle, CT/membership markers và source SHA trong web bundle. Evidence: `docs/evidence/p19-staging-public-smoke-20260909-b0263c9.json`. Evidence drift trước khi sửa được giữ tại `docs/evidence/p19-staging-public-smoke-20260909-drift-0098cf1.json`.
- Quy tắc vận hành: mọi commit chỉ thay đổi docs/root hoặc một service phải đi kèm parity marker/watch-path change, hoặc phải trigger deploy thủ công cho service còn lại; sau đó bắt buộc chạy verifier với exact SHA trước khi ghi staging parity PASS. Đây chỉ đóng public source-parity sub-gate, không đóng P17/P19.

### Checkpoint local Docker workload P17 — 2026-09-09

`scripts/verify-p17-docker-workload.ps1` đã được chạy trên API container sau khi rebuild đúng source. Hai benchmark job chạy đồng thời, mỗi job lặp 3 lần với synthetic RTDOSE/RTSTRUCT shape `64×128×128` (`1,048,576` voxel), không có patient data. Cả 6 kết quả dùng engine `p17-dvh-1.1.0`, cùng oracle `Dmean=2.5 Gy`, `Dmin=1 Gy`, `Dmax=4 Gy`, selected volume `12,582.912 cc`; API `/health=ok`, `/ready=ready`, schema `20260909_0018`. Lần chạy hiện tại có 1 mẫu `docker stats` max `142,396,621 bytes` (135.8 MiB) và cgroup v1 observation có current `148,406,272 bytes`, peak `367,915,008 bytes` kể từ lúc container start, không có cgroup limit hữu hạn trong local Docker; `is_peak_rss=false` bắt buộc giữ nguyên. Evidence: `docs/evidence/p17-local-docker-workload-20260909.json`.
...
- [x] P18-W00 — Local route-to-persistence integration pack tại `apps/api/tests/test_p18_integration.py`: QA journey và Biological journey **2/2 PASS** trên SHA `4ecfae8`; evidence mới nhất `docs/evidence/p18-local-integrated-20260911-4ecfae8.json`. Bao phủ auth/session, nested folder/case, Machine QA, artifact validation, Gamma queue/idempotency/worker, report export/download hash, trend rebuild và Biological library/BED-EQD2/comparison/re-irradiation/report không liên kết QA. Đây là `LOCAL_VERIFIED` support evidence, không thay staging/pilot/restore gate.
...
- [ ] P18-W02 — Fault injection API/DB/Redis/storage/worker/renderer trên staging có restore plan. `STAGING_PARTIAL_RESTART_RECOVERY`: same-commit worker restart/recovery và queue readback đã pass ở `docs/evidence/p18-staging-worker-restart-recovery-20260910-1c0210c.json`; API/DB/Redis/storage/renderer fault injection, retry/dead-letter và restore-plan execution còn mở.
...
- [x] P19-W02 — Effective settings matrix web/API/worker; migrations single runner, private DB/Redis. `STAGING_VERIFIED`: `scripts/verify-railway-effective-settings.ps1` đạt `passed=true`, `0` lỗi và evidence `docs/evidence/p19-railway-effective-settings-20260910.json`; chỉ ghi tên biến/phân loại endpoint, không ghi secret/database URL.
- [ ] P19-W03 — DNS/TLS/CORS/SPA fallback/Supabase redirects/build-time vars smoke. `STAGING_PARTIAL`: HTTPS, CORS origin, Vite API/Auth variables, SPA deep-link và authenticated session đã kiểm; custom DNS/redirect matrix và full external device matrix còn mở.
- [ ] P19-W04 — External browser/device journeys; version mismatch checks, rollback rehearsal and monitoring hooks. `STAGING_PARTIAL`: Edge authenticated readback + deep-link/reload của organization/QA/DVH/report/Biological/status đã ghi ở `docs/evidence/p19-staging-authenticated-remote-readback-20260910-3e79f48.json`; mutation E2E, second identity, export download, rollback rehearsal và monitoring hooks còn mở.
...
- [ ] P20-W01 — Tạo operational dashboard và alert test bằng sự kiện synthetic. Dashboard phải đọc riêng `/api/v1/health`, `/api/v1/ready`, `/api/v1/version` và, khi có session hợp lệ, `/api/v1/gamma/queue-metrics`; nếu `/ready` lỗi hoặc schema lệch thì trạng thái là degraded/needs review dù `/health` vẫn `ok`. **Local UI slice verified 2026-09-09:** `PlatformStatusPage` hiển thị từng probe, schema parity và trạng thái tổng hợp `SẴN SÀNG/CẦN XEM XÉT/API KHÔNG KHẢ DỤNG`; frontend 4 files/11 tests, lint, typecheck và build pass. **Local auto-refresh verified 2026-09-11:** các probe tự đọc lại mỗi 30 giây, có trạng thái đang cập nhật và thời điểm quan sát gần nhất; evidence `docs/evidence/p20-local-status-auto-refresh-20260911-7df7cb8.json`. **Staging source-parity verified 2026-09-11:** API/web/worker cùng SHA `3d764e40bcec98c229eb9992d4af85636c6cf784`, public verifier đạt `15/15`, schema `20260909_0019`; evidence `docs/evidence/p20-staging-status-refresh-public-20260911-3d764e4.json`. **Previous public parity checkpoint 2026-09-11:** candidate `d64e64ba5ac2f17b91d35c231a70092a81101a9c` đạt `15/15`, schema `20260909_0019`; evidence `docs/evidence/p20-staging-public-smoke-20260911-d64e64b.json`. Status page hiển thị `failed_runs` là `terminal history` và yêu cầu mở error snapshot để triage, không coi bộ đếm lịch sử là worker outage. **Operational probe verified 2026-09-11:** `scripts/verify-operational-probes.ps1` trên staging candidate `31021192c5dd4b726d8da2183d7e01d26d4e0df2` với schema `20260909_0019` đạt public checks PASS; negative expected-version control trả exit code `1` và fail đúng `api.version.expected`. Queue probe không chạy khi không có session token; alert tới kênh thật vẫn `NOT_RUN`.
- P20-W01 current read-only checkpoint (2026-09-11): `scripts/verify-operational-probes.ps1` trên candidate `f938fd7541fbe5c8086f12e2e0cfe3cd74dd6418`, schema `20260911_0020`, đạt `passed=true`, `failed_check_count=0`; queue `NOT_RUN` vì thiếu session token. Evidence `docs/evidence/p20-staging-operational-probes-20260911-f938fd7.json`. Alert delivery và các gate backup/restore vẫn chưa chạy.
...
> Các candidate, version và evidence được nêu trong đoạn snapshot bên dưới là lịch sử của revision v4.9; revision hiện hành và checkpoint staging mới nhất được ghi ở mục 7.3.

Đợt rebaseline ban đầu đã tạo BA v0.20, specification v1.14, technical-specification v1.11 và plan v3.5; các bản cập nhật kế tiếp được giữ trong lịch sử. Revision hiện hành của bộ tài liệu là BA v0.22, specification v1.19, technical-specification v1.18 và plan v4.9, bổ sung feature-card/handoff, operation/error/evidence record, dependency graph, change-impact gate, contract membership/invitation P4, source-identifiable Railway release metadata, P17 workload semantics, export-content evidence, targeted direct PostgreSQL row/checksum/scope evidence, object-storage byte re-hash, immutable-snapshot database guard và source-parity recovery runbook. Bộ tài liệu hiện có ma trận hành vi ở cấp tính năng, từ điển trạng thái, error taxonomy, operation/evidence contract, B01–B12 và ma trận bao phủ P0–P20. P16 đã có implementation local và staging browser smoke trên migration `20260908_0016`, còn direct PostgreSQL/scope/fault/release closure vẫn mở. P17 đã có pure engine/API/UI/migration, explicit P11/P16 limit binding, DVH report source và bounded CT preview local; staging đã có authenticated DVH/CT browser smoke với S12–S16, no-overlap warning, export-content parse/hash, targeted PostgreSQL row/checksum/scope probe, object-storage byte re-hash và migration `20260909_0019` trigger/negative probe, còn protocol compatibility, browser finalization, fault/volume, independent oracle promotion và release evidence vẫn mở. P18 hiện có local integrated journey, local browser support matrix và local backup/restore support nhưng staging fault/restore/pilot vẫn mở. Public staging hiện hành đã chứng minh schema head `20260909_0019`, source-label API/web parity trên candidate `b0263c932c740d4f36f19867241d5c1e07014765` và Auth boundary unauthenticated; evidence mới nhất nằm tại `docs/evidence/p19-staging-public-smoke-20260909-b0263c9.json`. Authenticated E2E, worker/production manifest và các production gates chưa đóng. P20 đã có runbook support artifact và local status dashboard slice nhưng alert/restore/owner evidence chưa có. Không phase nào được đánh dấu `DONE-v2` chỉ vì local test, HTTP 200 hoặc Railway báo Online.
...
`902b75729c97b6927465d70b93225d5702bdc83f`, evidence `docs/evidence/p19-staging-public-smoke-20260910-902b757.json`;
...
`902b75729c97b6927465d70b93225d5702bdc83f`, nhưng browser/Auth/DB/audit/scope/timeout evidence
...
lần chạy trước, không phải release hiện tại. Bổ sung trong v4.14: P10 có count-preflight,
raw/aggregate query budget theo Settings và CSV bucket lineage; candidate `85ecb0e` có browser
