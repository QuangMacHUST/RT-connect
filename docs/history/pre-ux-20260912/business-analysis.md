...
**Hợp đồng hình học và hệ tọa độ:** Mỗi cặp input dùng cho Gamma phải có physical basis được khai báo, không suy ra từ tên file hoặc thứ tự mảng. RTDOSE DICOM dùng `PATIENT_LPS` với `FrameOfReferenceUID`, storage order 2D `[y,x]` hoặc 3D `[z,y,x]`; measurement JSON phải khai báo `basis`, `frame_id`, `frame_of_reference_uid` khi dùng patient frame, `axis_order` và `transform_to_reference` có hướng `SOURCE_TO_REFERENCE`, đơn vị `mm` và nguồn/version/checksum. Reference và evaluation chỉ được so sánh khi basis, frame identifier, axis order và transform tương thích. Capability hiện tại chỉ hỗ trợ transform identity đã khai báo; transform tịnh tiến/rigid/oblique/non-uniform chưa có adapter thì phải báo chưa hỗ trợ, không tự căn chỉnh hoặc đổi trục. Tương thích JSON legacy chỉ dành cho các dòng ENGINE_TEST nội bộ đã tồn tại; upload measurement mới không được bỏ qua block này.
...
**Quy tắc dữ liệu lớn:** Chế độ `raw` mặc định chỉ đọc tối đa `TREND_MAX_RAW_POINTS=10.000` source points. Chế độ `day` hoặc `week` được phép đọc tối đa `TREND_MAX_AGGREGATE_SOURCE_POINTS=100.000` source points để tạo bucket. API phải đếm trước theo organization, machine, time range và filter tương thích; vượt giới hạn trả `TREND_QUERY_TOO_LARGE` với số điểm khớp, giới hạn đang áp dụng và hướng dẫn thu hẹp phạm vi/chuyển aggregate. Aggregate vượt raw budget phải hiển thị cảnh báo rõ ràng, nhưng vẫn giữ count, cực trị và toàn bộ source point/run IDs để drill-down. Không được tự cắt dữ liệu, lấy mẫu im lặng hoặc trả CSV thành công nhưng rỗng.

**Luồng chính:** Chọn machine, metric và khoảng thời gian. → Chọn filter và nhóm tương thích. → Hiển thị điểm raw hoặc bucket day/week theo budget. → Chọn điểm/bucket để mở case/run/report nguồn. → Export cùng bộ lọc, timezone và phương pháp aggregate; export aggregate phải có record bucket, thống kê và lineage nguồn.

**Nghiệm thu nhóm:** Không trộn máy/unit; baseline/outlier/timezone/filter/export/drill-down và rebuild pass; raw vượt budget bị chặn trước khi materialize, aggregate lớn có cảnh báo và export không mất lineage.
...
| FR-P12-04 | Tạo report độc lập từ scenario revision; nguồn, assumption và provenance luôn tra lại được. |
...
**Luồng chính:** Vào tab Biological Toolkit. → Chọn công cụ hoặc mở scenario cũ. → Nhập dữ liệu thủ công hoặc dataset riêng do user chọn. → Lưu scenario và calculation revision. → Chọn `Tạo report` từ scenario đang mở. → Hệ thống tạo report revision với source snapshot của đúng organization và scenario revision hiện tại. → Mở Report Builder để toàn quyền sắp xếp/chỉnh sửa layout theo chính sách sản phẩm. → Mở lại, clone hoặc export calculation report độc lập.
...
| FR-P17-05 | Người dùng có thể chủ động chọn đúng một nguồn giới hạn: một `DOSE_LIMIT` từ Biological Knowledge Library hoặc một rule `ACTIVE` từ QA Protocol; hệ thống hiển thị actual/limit/margin/status và source snapshot, không tự tìm, xếp hạng hoặc áp dụng giới hạn. |
| FR-P17-06 | Report Builder có thể nhận một DVH run đã lưu làm source; report giữ nguyên result/input/limit snapshot của run và không chạy lại hoặc trỏ sang “latest” khác. |
| FR-P17-07 | CT anatomy preview độc lập với phép tính DVH: đọc HU, chọn lát cắt, window/level, dose overlay và ROI overlay theo patient LPS khi geometry/Frame of Reference tương thích; hiển thị transform, crosshair, coverage và warning rõ ràng. |
...
- CT preview ở phiên bản này nhận một file CT DICOM đơn hoặc multi-frame có geometry top-level đầy đủ, ảnh một kênh `MONOCHROME1`/`MONOCHROME2`, rescale sang HU và window/level. Người dùng có thể chọn `frame_index`; hệ thống tạo lưới hiển thị bị giới hạn, dose overlay nearest-neighbor và ROI overlay tùy chọn. CT series nhiều file, Enhanced CT chỉ có geometry trong functional groups, registration ngoại lai/deformable và resampling chất lượng chẩn đoán chưa thuộc capability này.
- CT preview không làm thay đổi dose values, DVH, prescription hay QA result. Kết quả preview là read-only, có `schema_version`, `engine_key/version`, source grid/output grid, frame/spacing/window, patient-LPS mapping, crosshair, valid/outside pixel counts, warning và `result_sha256`. Nếu lát CT không giao với dose grid, vẫn có thể xem CT nhưng overlay dose phải rỗng và có warning `CT_DOSE_NO_OVERLAP`; không gán ngoài vùng thành zero.
...
- Nguồn giới hạn là tùy chọn và phải được chọn rõ trong cùng request: `limit_entry_id` cho một entry `DOSE_LIMIT` hoặc `protocol_version_id` cùng `protocol_metric_key` cho một protocol `ACTIVE`; không được gửi đồng thời cả hai. Không có nguồn thì kết quả chỉ là DVH, không có actual/limit/margin.
- P16 `DRAFT` chỉ tạo warning khi user chủ động chọn để preview; `ARCHIVED`, entry sai loại, protocol không `ACTIVE`, rule không tồn tại hoặc source khác organization đều chặn. Override chỉ áp dụng cho entry P16, phải nằm trong whitelist và được snapshot cùng effective values; protocol không nhận override tự do.
- Metric giới hạn phải có trong result của request: `DMIN`, `DMEAN`, `DMAX`, `Dxx` cần `dx_percentages`, `Vx` cần `vx_doses_gy` và `metric_parameter`; `Dxcc` chưa được bật trong P17. Unit của actual và limit phải khớp, không chuyển đổi ngầm.
...
7. Nếu user chọn một protocol rule hoặc knowledge limit, hệ thống validate nguồn trong organization hiện tại, tính actual/limit/margin, gắn `rule_status` và snapshot nguồn vào result; warning của source vẫn hiển thị và không bị biến thành PASS.
8. User có thể chọn DVH run đã lưu trong Report Builder; hệ thống snapshot toàn bộ DVH result và provenance vào report source, không tự tìm run khác và không tính lại.
9. Nếu user chọn CT, hệ thống tải lại dose/CT/RTSTRUCT theo scope và checksum, giải mã HU, chọn lát cắt và window/level, map từng pixel CT sang dose grid bằng patient LPS, rồi hiển thị grayscale + dose/ROI overlay + crosshair. Đổi lát cắt chỉ tạo preview read-only mới; không tạo DVH run hay report revision.
...
| TC-P17-S09 | Chọn một P16 `DOSE_LIMIT` tương thích, metric đã yêu cầu đủ | Result có source/effective snapshot, actual/limit/margin, `rule_status` và hash; không thay đổi DVH engine result gốc ngoài lớp binding. |
| TC-P17-S10 | Chọn một P11 protocol `ACTIVE` và rule `d95` | Rule được resolve theo organization và metric key, result hiển thị actual/limit/margin; protocol version/rule snapshot giữ nguyên sau khi protocol thay đổi về sau. |
| TC-P17-S11 | Chọn DVH run đã lưu làm source trong Report Builder | Report tạo được từ run đúng case/org, source snapshot chứa result/limit evaluation; refresh/export không rerun DVH. |
| TC-P17-S12 | Chọn RTDOSE + CT cùng Frame of Reference, CT single-file hợp lệ | Preview trả HU, window/level, source/output grid và `result_sha256`; dose overlay có số pixel valid/outside; DVH và dose values không bị thay đổi. |
| TC-P17-S13 | CT multi-frame và đổi `frame_index` hợp lệ | Lát CT, slice offset và display pixels thay đổi đúng frame; mapping/crosshair được tính lại deterministic; không tạo persistence side effect. |
...
| TC-P17-E09 | CT thiếu khi muốn overlay, CT sai frame/geometry | `DVH_DOSE_ONLY_MODE`, `DICOM_FRAME_MISMATCH`, `DVH_ANATOMY_ARTIFACT_INVALID` | Cho phép dose-only nếu DVH hợp lệ; tắt overlay và ghi lý do, không vẽ anatomy sai. |
...
| TC-P17-E18 | Gửi đồng thời `limit_entry_id` và `protocol_version_id`, hoặc gửi override không có source | `DVH_LIMIT_BINDING_CONFLICT`, `DVH_LIMIT_OVERRIDE_INVALID` | Validate false/422 tùy operation; không chạy engine và không tạo run; giữ input để chọn một nguồn rõ ràng. |
| TC-P17-E19 | Entry không tồn tại/ARCHIVED/sai loại hoặc không thuộc organization | `DVH_LIMIT_ENTRY_NOT_FOUND`, `DVH_LIMIT_NOT_AVAILABLE`, `DVH_LIMIT_ENTRY_INVALID` | Boundary-safe; không lộ entry khác org, không fallback sang limit khác; chọn entry mới hoặc tạo version hợp lệ. |
| TC-P17-E20 | Protocol không tồn tại/không ACTIVE/thiếu rule hoặc rule type chưa hỗ trợ | `DVH_PROTOCOL_NOT_FOUND`, `DVH_PROTOCOL_NOT_AVAILABLE`, `DVH_PROTOCOL_RULE_REQUIRED`, `DVH_PROTOCOL_RULE_NOT_FOUND`, `DVH_PROTOCOL_RULE_UNSUPPORTED` | Chặn binding; chọn protocol/rule ACTIVE tương thích hoặc tạo version mới; không dùng rule gần giống. |
| TC-P17-E21 | Metric không có trong Dx/Vx request, Dxcc chưa hỗ trợ hoặc unit actual/limit khác nhau | `DVH_LIMIT_METRIC_NOT_COMPUTED`, `DVH_LIMIT_METRIC_UNSUPPORTED`, `DVH_LIMIT_UNIT_MISMATCH` | Không tạo evaluation giả; bổ sung metric request/chọn rule khác hoặc sửa unit nguồn có version; không đổi unit ngầm. |
| TC-P17-E22 | Limit numeric/range/operator/override không hợp lệ hoặc source warning cần review | `DVH_LIMIT_DEFINITION_INVALID`, `DVH_LIMIT_ENTRY_INVALID` và warning source | Lỗi definition chặn; warning vẫn giữ result nhưng `status=REVIEW_REQUIRED`, `rule_status` tách riêng PASS/FAIL. |
| TC-P17-E23 | DVH run không thuộc org/case hoặc Report Builder gửi source type/ID không khớp | `REPORT_SOURCE_UNAVAILABLE`, `DVH_RUN_NOT_FOUND` | Không tạo report từ nguồn ngoài scope; chọn run đúng case/org, giữ report draft và không lộ metadata. |
...
**Nghiệm thu nhóm:** P17 chỉ đạt khi có known-answer geometry/DVH, API scope/checksum/idempotency, explicit P11/P16 binding, DVH report source, UI loading/empty/warning/error/result/history/export và staging browser→API→Railway PostgreSQL/object storage trên cùng candidate. CT anatomy phải đạt S12–S16 và E24–E30; workload/fault và independent oracle phải có evidence riêng nếu nằm trong release scope. P17 không được coi là hoàn tất chỉ vì endpoint health trả 200 hoặc ảnh Stitch hiển thị đúng.
...
| P8 | Chạy PSQA Gamma 2D/3D từ input hợp lệ và theo dõi job | Chọn RTDOSE/reference/comparison → preflight profile, frame, axis order, transform, unit, config và resource → enqueue → worker lease/heartbeat → tính/lưu → ack → xem map/histogram/profile → retry/rerun/compare | Thiếu RTDOSE hoặc comparison, thiếu FrameOfReferenceUID/coordinate block, geometry/axis/frame/transform/unit không tương thích, transform ngoài capability, config unsupported, không có evaluated points, local reference 0, Redis lỗi, worker chết/OOM/lease hết, DICOM/codec unsupported, quá resource, source đổi | PSQA phải có RTDOSE; JSON-only phải mang nhãn ENGINE_TEST; input mới không được thiếu physical frame; không trộn legacy với explicit/DICOM; FULL_ROI không bỏ no-candidate khỏi mẫu số; OVERLAP_ONLY phải nêu coverage; max gamma phải nêu censoring; worker cũ không ghi đè |
...
| P8 | Preflight → accepted/outbox → Redis/lease → Gamma → result → retry/compare | `TC-P08-S01..S05` | `TC-P08-E01..E11` | 2D/3D oracle, denominator, coordinate-frame/axis/transform compatibility, attempt/fencing, fault/resource evidence. |
...
### 22.1. Quy ước áp dụng cho mọi tính năng

- **Người thực hiện:** active member của organization. Bác sĩ, kỹ sư vật lý và thành viên chuyên môn ngang nhau trong phạm vi nghiệp vụ; không tạo action role, approval hierarchy hoặc luồng khóa thao tác theo chức danh.
- **Ngữ cảnh:** mọi dữ liệu nghiệp vụ phải được mở trong đúng `organization context`; resource được tham chiếu phải còn tồn tại, đúng parent và đúng trạng thái cho thao tác.
- **Tách trạng thái:** trạng thái màn hình (`loading`, `empty`, `ready`, `warning`, `error`, `conflict`) tách với trạng thái lưu/tính (`DRAFT`, `ACCEPTED`, `RUNNING`, `COMPLETED`, `FAILED`, `ARCHIVED`) và kết quả chất lượng (`PASS`, `WARNING`, `FAIL`, `N/A`).
- **Thành công có thể quan sát:** một thao tác đúng phải trả ra kết quả người dùng thấy được, định danh/revision, thời điểm, nguồn và bước tiếp theo. HTTP 200, toast hoặc màu xanh riêng lẻ không phải bằng chứng hoàn thành.
- **Lỗi có thể phục hồi:** lỗi phải nói rõ người dùng cần sửa field, chọn lại nguồn, tải lại trạng thái, retry, tạo revision mới hay liên hệ vận hành. Không được dùng “Not Found” cho mọi lỗi membership, không được để spinner vô hạn và không được hiện stack trace/secret.
- **Dữ liệu không bị mất:** retry, refresh, đổi route, đổi tên, move, archive, đổi protocol/model/template hoặc worker restart không được làm mất source, result, revision, history hay provenance đã commit.
- **Không đoán ngầm:** không tự đổi unit, tự ghép dataset, tự chọn baseline/limit, tự sửa số nhập, tự bỏ điểm lỗi hoặc tự tạo organization khi nguyên nhân thật là outage/configuration.
- **Bằng chứng tối thiểu:** testcase, commit, environment, fixture/hash, expected/observed, trạng thái DB/object/queue nếu có, screenshot/response đã redaction và hành động phục hồi. Testcase mới phát hiện trong pilot phải trở thành regression case.

### 22.2. Thẻ hoàn thiện bắt buộc cho một feature

Trước khi đánh dấu một `FR` là hoàn thiện, phase owner phải điền đủ các trường sau trong phase packet và liên kết sang `plan.md`/`specification.md`:

| Trường | Câu hỏi phải trả lời | Nếu thiếu thì xử lý thế nào |
| :--- | :--- | :--- |
| Goal | Người dùng đang cố hoàn thành việc gì và kết quả có ý nghĩa gì? | Chưa được triển khai UI/API để gọi là feature. |
| Actor/context | Ai thực hiện, đang ở organization/site/machine/case/scenario nào? | Bổ sung context; không dùng global lookup hoặc dữ liệu demo thay thế. |
| Preconditions | Những resource, version, file, capability, Auth và service nào phải sẵn sàng? | Ghi `BLOCKED` hoặc `UNAVAILABLE`, không cho nút hoạt động giả. |
| Main flow | Các bước từ mở màn hình đến kết quả cuối là gì? | Viết thành testcase `TC-Pxx-Syy` và browser/API flow. |
| Valid variants | Zero, optional, nhiều dòng, biên hợp lệ, refresh/reconnect và rerun xử lý ra sao? | Bổ sung B02/B03/B11 hoặc testcase đặc thù. |
| Invalid/error flow | Sai field, quan hệ chéo, scope, archive, duplicate, conflict và dependency fail xử lý ra sao? | Bổ sung `TC-Pxx-Eyy`, error code và recovery. |
| Side effect | Resource/revision/object/job/audit nào được tạo hoặc không được tạo? | Kiểm DB/object/queue; không chấp nhận chỉ kiểm response. |
| Invariant | Điều gì tuyệt đối không được thay đổi hoặc lộ ra? | Thêm assertion về checksum, source, organization, version và history. |
| UI states | Loading/empty/ready/warning/error/conflict/offline có nội dung và hành động nào? | Thiết kế/implement đủ state trước visual acceptance. |
| Evidence | Có thể chứng minh kết quả bằng ID/hash/version nào? | Giữ feature ở `NOT_RUN`, `LOCAL_VERIFIED` hoặc `STAGING_VERIFIED`; không tự nâng trạng thái. |

### 22.3. Ma trận chi tiết từng phase và nhóm tính năng

Bảng sau là bản đồ nghiệp vụ cấp feature. “Chạy đúng” là luồng phải làm được; “biên/lỗi” là các tình huống phải có hành vi rõ; “bất biến/bằng chứng” là điều phải kiểm sau thao tác. Các mã `S/E` được dùng lại trong `plan.md`; nếu code thực tế dùng error code khác cùng nghĩa, phải cập nhật mapping trong cùng change set.

| Phase / FR | Workflow và tính năng phải làm được | Chạy đúng và biến thể hợp lệ | Lỗi/ngoại lệ và phục hồi bắt buộc | Bất biến, output và bằng chứng |
| :--- | :--- | :--- | :--- | :--- |
| **P0** `FR-P00-01..04`<br>Baseline và truy vết | Đọc yêu cầu → lập registry BR/FR/MOD/route/design/contract/test → ghi gap → quyết định phạm vi → khóa baseline. | Tài liệu nhất quán; thêm requirement không làm đổi lịch sử; tiếp tục sau gián đoạn từ checkpoint. | Mâu thuẫn tài liệu, Stitch screen hidden, link test/evidence hỏng, sai project/environment: ghi decision hoặc gap, giữ phase mở; không chữa bằng cách xóa requirement/evidence. | Version các tài liệu, decision log, registry và hash source. Kiểm 100% FR có owner/phase/contract/S/E/evidence target; `DOCUMENT_CONFLICT`, `DESIGN_REFERENCE_STALE`, `EVIDENCE_MISSING`, `ENVIRONMENT_MISMATCH`. |
| **P1** `FR-P01-01..04`<br>Runtime và CI | Clone sạch → cài lockfile → khởi động PostgreSQL/Redis/object storage → migrate/seed synthetic → chạy API/web → test/lint/build/OpenAPI → restart. | DB rỗng và DB đã có revision đều upgrade được; restart giữ dữ liệu và seed không nhân bản; CI xanh trên đúng SHA. | Thiếu dependency, lock lệch, port bận, env thiếu, migration fail, build/OpenAPI lệch: nêu đúng layer, dừng release, sửa rồi chạy lại từ checkpoint; không seed production. | Setup report, migration head, build artifact, CI run ID, log redaction. `DEPENDENCY_MISMATCH`, `PORT_IN_USE`, `MIGRATION_FAILED`, `CONFIGURATION_MISSING`, `BUILD_CONTRACT_FAILED`. |
| **P2** `FR-P02-01..04`<br>Staging/Auth/DB | Đối chiếu project/environment/service → build đúng root/SHA → pre-deploy migration → health → ready/schema → kiểm JWT Supabase → ghi manifest. | `postgresql://` và `postgresql+psycopg://` cùng dùng psycopg 3; API staging đọc PostgreSQL staging; health, readiness/schema và token hợp lệ đều đúng; lỗi token bị từ chối. | Railpack/source/root sai, `psycopg2` thiếu, bind/PORT/health sai, DB chưa migrate, issuer/audience/JWKS sai, effective config drift: giữ last-good, sửa đúng service/environment, retry bounded; không kết luận từ Online/health riêng. | Service ID, deployment SHA, schema revision, Auth issuer/audience (không ghi secret), health/ready/version response. `BUILD_SOURCE_INVALID`, `DATABASE_DRIVER_MISMATCH`, `SERVICE_NOT_LISTENING`, `SCHEMA_NOT_READY`, `AUTH_VERIFICATION_FAILED`, `DEPLOYMENT_CONFIG_DRIFT`. |
| **P3** `FR-P03-01..04`<br>App shell/Auth/Home | Deep-link → login/recovery/callback → bootstrap identity → existing membership vào workspace hoặc first-use onboarding → Home → logout/expiry. | Session refresh single-flight; return URL chỉ tới route nội bộ; dashboard rỗng có CTA; widget populated dùng dữ liệu thật; keyboard/mobile và tiếng Việt đọc được. | Sai login, recovery hết hạn, session/JWKS outage, membership thiếu/inactive, VITE config/CORS/API sai, offline: giữ email nhưng không giữ password, không tự gán organization; cho retry/login/onboarding đúng nguyên nhân. | Cache key theo identity/org; logout xóa dữ liệu nhạy cảm; bootstrap response và browser state matrix. `SIGN_IN_FAILED`, `RECOVERY_LINK_INVALID`, `SESSION_UNAVAILABLE`, `ORGANIZATION_MEMBERSHIP_REQUIRED`, `AUTH_CONFIGURATION_MISSING`, `WORKSPACE_LOAD_FAILED`. |
...
| **P8** `FR-P08-01..04`<br>PSQA Gamma | Chọn RTDOSE reference + comparison đã VALID → preflight profile/geometry/frame/axis/transform/config → accept/outbox → Redis/worker lease → Gamma → persist map/statistics/profile → refresh/retry/compare. | PSQA có RTDOSE; ENGINE_TEST JSON-only được gắn nhãn riêng; 2D/3D, global/local, absolute/relative và coverage explicit; result có denominator, counts, map, config/engine snapshot và physical-frame snapshot. | Thiếu RTDOSE/comparison, thiếu coordinate block/FoR, frame/grid/axis/transform/unit sai hoặc transform ngoài capability, config unsupported, no candidate/local zero, Redis/worker/lease/OOM/timeout, source drift: chặn hoặc retry bounded/dead-letter; worker cũ không commit; không bỏ điểm để tăng pass. | Run/attempt/lease/outbox, Gamma oracle, coverage/max-gamma/censoring, input/config/frame/transform/engine version. `RTDOSE_REQUIRED`, `COMPARISON_REQUIRED`, `GAMMA_COORDINATE_FRAME_MISSING`, `GAMMA_COORDINATE_FRAME_INVALID`, `GAMMA_AXIS_ORDER_UNSUPPORTED`, `GAMMA_TRANSFORM_UNSUPPORTED`, `GAMMA_INPUT_INCOMPATIBLE`, `GAMMA_CONFIG_UNSUPPORTED`, `GAMMA_NO_EVALUATED_POINTS`, `GAMMA_LOCAL_ZERO_REFERENCE`, `GAMMA_DISPATCH_UNAVAILABLE`, `GAMMA_EXECUTION_INTERRUPTED`, `GAMMA_RESOURCE_LIMIT`, `GAMMA_SOURCE_CHANGED`. |
...
| **P10** `FR-P10-01..04`<br>Trend/baseline/maintenance | Chọn machine/metric/timezone → lọc context tương thích → xem raw/aggregate/baseline/limit/maintenance marker → drill-down source → export/rebuild. Raw bị giới hạn trước khi tải; day/week là đường xử lý cho tập lớn có budget riêng. | Không trộn machine/unit/energy/context; date end inclusive được chuẩn hóa; empty là empty; aggregate giữ count/extrema/source IDs; rebuild lặp lại không nhân đôi; CSV aggregate có bucket record và lineage. | Date/timezone/filter sai, incompatible series, baseline thiếu/zero, source duplicate/archived/unavailable, query lớn, event conflict: trả field/warning/aggregate phù hợp; không tạo zero giả, nối series sai, cắt dữ liệu im lặng hoặc trả file rỗng. | Compatibility signature, source IDs, timezone/aggregation, baseline/event revisions, export snapshot và query budget. `TREND_SERIES_INCOMPATIBLE`, `DATE_RANGE_INVALID`, `TREND_TIMEZONE_INVALID`, `TREND_FILTER_INVALID`, `TREND_EMPTY`, `TREND_BASELINE_INVALID`, `TREND_SOURCE_UNAVAILABLE`, `TREND_QUERY_TOO_LARGE`, `MAINTENANCE_EVENT_CONFLICT`. |
| **P11** `FR-P11-01..04`<br>QA Protocol Library | Search → detail → create/clone → sửa applicability/rule/reference → validate-only → lưu DRAFT/version → activate/archive → chọn explicit cho run mới → compare. | Clone deep-copy; reference/source và changelog hiển thị; version đã dùng không đổi; consumer chỉ chọn capability/rule được hỗ trợ; report cũ giữ snapshot. | Rule min/max/deviation vô nghĩa, thiếu source, key/version trùng, archived/unsupported, concurrent edit, persistence uncertain: field error/conflict, clone version mới, query/reconcile; không drop rule hoặc update ngược run cũ. | Protocol/version/rule snapshot, source citation, consumer capability, migration `20260908_0011`, compare evidence. `PROTOCOL_RULE_INVALID`, `PROTOCOL_SOURCE_REQUIRED`, `PROTOCOL_VERSION_CONFLICT`, `PROTOCOL_NOT_AVAILABLE`, `PROTOCOL_CAPABILITY_MISMATCH`, `PROTOCOL_PERSISTENCE_FAILED`. |
| **P12** `FR-P12-01..04`<br>Biological Hub | Mở tab riêng → xem capability → tạo scenario → sửa/save/clone/archive → mở calculation history → export calculation report. | Scenario không cần QA case/patient; tool chưa có phải hiện PLANNED/UNAVAILABLE; clone giữ lineage; calculation lưu model/assumptions/source. | Auth/scope/session, key/context/revision sai, module/model unavailable, conflict, persistence/export fail: giữ form hoặc history, retry sau query; không auto-link QA, không tạo result giả. | Scenario/revision/calculation IDs, namespace độc lập, source/model/assumption snapshot. `SESSION_UNAVAILABLE`, `BIOLOGICAL_SCENARIO_INVALID`, `SCENARIO_REVISION_NOT_FOUND`, `SCENARIO_IMMUTABLE`, `MODULE_UNAVAILABLE`, `CALCULATION_PERSISTENCE_FAILED`. |
| **P13** `FR-P13-01..04`<br>BED/EQD2 và đồ thị | Chọn saved scenario revision → nhập D/n/d + tissue/alpha-beta/source → validate-only → calculate → chọn curve D/range/step/alpha-beta → bảng/đồ thị/marker → save/replay/export. | Known-answer đúng; zero dose phân biệt với thiếu; fixed-n/fixed-d và nhiều series; table/chart/CSV dùng cùng dataset/checksum; display rounding không đổi engine precision. | n/D/d/alpha-beta sai, D khác n×d, non-finite/overflow, thiếu source, range/step/point limit, archived revision, idempotency/persistence fail: field error hoặc `valid=false`, không clamp/đổi input/ghi chart một phần; query key trước retry. | Input/result/model version, formula explanation, curve schema, point count/hash, history/export. `BIOLOGICAL_INPUT_INVALID`, `FRACTIONATION_INCONSISTENT`, `CALCULATION_NONFINITE`, `CURVE_RANGE_INVALID`, `ALPHA_BETA_SOURCE_REQUIRED`, `CALCULATION_IDEMPOTENCY_CONFLICT`. |
| **P14** `FR-P14-01..04`<br>So sánh phác đồ | Tạo 2–10 option từ input/P13 snapshot → chọn common tissue/model/context và baseline → validate compatibility → tính absolute/% delta → chart/table → reorder/clone/export/history. | Mỗi option giữ input; reorder không đổi option ID; same context cho phép compare; baseline zero trả delta % null có reason; option dài không bị truncate. | Thiếu/sai option, calculation chưa complete/out-of-scope, context/alpha-beta mismatch, baseline missing/zero, duplicate key/persistence: giữ option hợp lệ, warning hoặc reject đúng mức; không biến mismatch thành số so sánh được. | Option/result snapshot, baseline, delta policy, compatibility warning, chart/table checksum, no-QA linkage. `COMPARISON_OPTIONS_REQUIRED`, `COMPARISON_OPTION_INVALID`, `COMPARISON_CONTEXT_MISMATCH`, `COMPARISON_ALPHA_BETA_MISMATCH`, `BASELINE_ZERO`, `COMPARISON_IDEMPOTENCY_CONFLICT`. |
| **P15** `FR-P15-01..04`<br>Re-irradiation/bù fraction | Nhập course theo thời gian → chọn mô/model/alpha-beta → chọn no-recovery hoặc recovery explicit → tính cumulative scalar/sensitivity → nhập delivered/remaining/interruption → tạo alternative schedules → compare/export. | Nhiều course, fraction không đều, recovery/time model và sensitivity; compensation chỉ là proposal/scenario; assumption/source hiển thị; scalar không giả là spatial dose. | Ngày/interval/lịch không hợp lệ, thiếu recovery/source/context, non-integer, course overlap, spatial registration/OAR thiếu, persistence/export fail: chặn field hoặc warning; spatial `UNAVAILABLE`, tạo scenario mới; không sửa prescription/RTPLAN. | Course/input/assumption/recovery/source/model snapshot, scalar-vs-spatial capability, alternative schedule và independent export. `COURSE_INTERVAL_INVALID`, `RECOVERY_ASSUMPTION_INVALID`, `FRACTION_SCHEDULE_INVALID`, `SPATIAL_DATA_REQUIRED`, `SPATIAL_CAPABILITY_UNAVAILABLE`, `REIRRADIATION_PERSISTENCE_FAILED`. |
| **P16** `FR-P16-01..04`<br>Dose limits/protocol/knowledge | Search/filter disease/anatomy/OAR/technique/fraction → xem source/applicability → tạo/clone entry → validate/import preview → publish/archive/version → chọn explicit-use snapshot trong tool → compare/export. | Metric/operator/unit/volume có nghĩa rõ; citation và applicability hiển thị; import có row result; published version immutable; user override và draft được gắn nhãn; tool nhận snapshot do user chủ động chọn. | Thiếu source, metric/unit/operator/volume sai, link chưa verify, unsafe content, no match, duplicate/import conflict, stale revision, wrong scope/persistence: sửa row/clone/refresh/reconcile; không tự chọn limit, không biến entry thành PASS/FAIL. | Entry/version/citation/content hash, applicability/effective values, import report, explicit-use snapshot. `KNOWLEDGE_SOURCE_REQUIRED`, `DOSE_LIMIT_UNIT_INVALID`, `KNOWLEDGE_CONTENT_INVALID`, `REFERENCE_LINK_UNAVAILABLE`, `KNOWLEDGE_IMPORT_INVALID`, `KNOWLEDGE_VERSION_IMMUTABLE`, `KNOWLEDGE_PERSISTENCE_FAILED`. |
| **P17** `FR-P17-01..06`<br>Visual Dose/DVH | Chọn case → resolve VALID RTDOSE/RTSTRUCT/optional CT → geometry/manifest/checksum preflight → dose-native hoặc anatomy overlay → chọn ROI/policy/metric → optional explicit P11/P16 limit source → validate preview → save DVH run → history/export → chọn run làm report source. | Dose-native chạy không cần CT; DVH dùng ROI theo ROINumber; FULL_ROI/OVERLAP_ONLY explicit; Dmin/Dmean/Dmax/Dx/Vx và weighted volume cùng snapshot; binding có actual/limit/margin và `rule_status`; CT overlay chỉ bật khi frame/hình học compatible; report không rerun. | Thiếu/wrong artifact, manifest/checksum drift, unit/scaling/grid/frame/orientation/ROI/contour lỗi, coverage thiếu, metric/resource, binding source/rule/override/unit conflict, report source scope/id lỗi, idempotency/persistence: chặn hoặc warning theo policy, không clip/gán zero/đoán transform/fallback limit; retry sau reconcile, dose-only fallback có nhãn. | Input/result/binding checksum, geometry/ROI/coverage, engine/schema version, result hash, source/effective values, report snapshot. `DVH_INPUT_NOT_VALIDATED`, `DVH_SOURCE_CHANGED`, `DICOM_GEOMETRY_INVALID`, `DVH_ROI_INVALID`, `CONTOUR_GEOMETRY_INVALID`, `DVH_INCOMPLETE_COVERAGE`, `DVH_PARTIAL_COVERAGE`, `DVH_RESOURCE_LIMIT`, `DVH_LIMIT_BINDING_CONFLICT`, `DVH_LIMIT_METRIC_NOT_COMPUTED`, `DVH_LIMIT_UNIT_MISMATCH`, `REPORT_SOURCE_UNAVAILABLE`. |
| **P18** `FR-P18-01..04`<br>Integrated/reliability/pilot | Khóa RC manifest → chạy QA/Biological journeys → golden/reference → restart/offline/concurrency/fault/load → backup/restore → pilot dataset được phép → regression/triage. | E2E đi từ UI tới DB/object/queue/worker; restore giữ counts/checksum/source; performance đo theo workload; lỗi pilot có issue và test hồi quy; candidate có version thống nhất. | Golden lệch, restore thiếu object, duplicate replay, latency/memory/capacity fail, dataset capability gap, evidence sai SHA: chặn RC, giữ evidence/fixture, mở root-cause issue và rerun; không sửa expected để ép PASS. | RC manifest, test matrix, workload, fault injection log, RPO/RTO/checksum, severity/issue/regression. `RESULT_REGRESSION`, `RESTORE_INCOMPLETE`, `DUPLICATE_RESULT`, `PERFORMANCE_GATE_FAILED`, `PILOT_CAPABILITY_GAP`, `RELEASE_EVIDENCE_MISMATCH`. |
| **P19** `FR-P19-01..04`<br>Production/remote access | Backup/restore point → promote API/web/worker/schema cùng RC → DNS/TLS/CORS/Auth/build config → remote browser E2E → monitor → rollback rehearsal. | HTTPS từ mạng ngoài; login, upload, job, report/download, toolkit; API/worker/DB/Redis versions tương thích; public web không chứa secret; rollback giữ dữ liệu. | DNS/TLS chưa ready, VITE config sai, migration/schema mismatch, web/API SHA lệch, health 200 nhưng workflow fail, budget vượt: không promote partial, giữ last-good, rollback theo manifest hoặc sửa config rồi kiểm lại. | Public URL, service/image SHA, schema/engine/renderer, Auth/CORS, backup/rollback record, remote request IDs. `PUBLIC_DOMAIN_NOT_READY`, `PUBLIC_BUILD_CONFIG_MISMATCH`, `RELEASE_SCHEMA_FAILED`, `RELEASE_VERSION_MISMATCH`, `REMOTE_E2E_FAILED`, `RESOURCE_BUDGET_EXCEEDED`. |
| **P20** `FR-P20-01..04`<br>Operations/continuous improvement | Setup dashboard/alert → test alert → backup/restore drill → runbook/support guide → incident triage → maintenance release qua staging → regression/release note. | Alert tới kênh thật; backup có manifest/retention; người vận hành tự restore và xử lý issue; engine update giữ old result và có diff; backlog có owner. | Backup/alert/retention/capacity fail, incident lặp lại, engine đổi số không giải thích được: giữ last-good, chạy lại/đổi kênh, RCA + regression + runbook update; không xóa hồ sơ tùy ý. | Alert timestamp, backup/restore checksum/RPO/RTO, owner/runbook version, incident severity, maintenance SHA và result diff. `BACKUP_POLICY_FAILED`, `ALERT_DELIVERY_FAILED`, `CAPACITY_WARNING`, `ENGINE_RESULT_CHANGED`, `RECURRING_INCIDENT`. |

### 22.4. Quan hệ giữa feature, UI, API, dữ liệu và testcase

Một feature được coi là có implementation đầy đủ khi cùng một `FR` có đủ năm liên kết sau:

1. **UI/route:** người dùng mở được màn hình thật, thấy đúng organization/context và thực hiện được main flow. Module nền không có màn hình riêng phải có route/API hoặc evidence vận hành tương ứng; không dùng ảnh Stitch thay cho flow.
2. **Command/API:** request, response, HTTP status, error code, retryability và operation ID được định nghĩa; API kiểm scope từ lookup đầu tiên.
3. **Persistence/engine:** dữ liệu được commit đúng transaction; output có input/config/model/engine/version snapshot; file/object/queue được đối soát khi có nhiều hệ thống.
4. **Testcase/evidence:** ít nhất có success, boundary/empty, field/cross-field error, scope/archive, duplicate/unknown outcome và output/provenance; phase có job/file/renderer/geometry phải thêm failure injection/volume/oracle tương ứng.
5. **Handoff:** `business-analysis.md` nêu mục đích/quy tắc; `specification.md` nêu contract; `technical-specification.md` nêu boundary/storage/topology; `plan.md` nêu thứ tự task, testcase, evidence và exit. Thay đổi một lớp mà không cập nhật ba lớp còn lại là `DOCUMENT_CONFLICT`.

Không được chuyển một feature sang production nếu một trong năm liên kết chỉ có mock, dữ liệu seed không được đánh dấu synthetic, route không có deep-link, API không có error contract, hoặc evidence không cùng SHA/schema/config với release. Với Biological Toolkit, “hoàn thiện” vẫn không có nghĩa là hệ thống đã quyết định điều trị; tool chỉ tạo calculation/scenario/reference có assumption và provenance.

### 22.5. Quy tắc thay đổi và bổ sung trường hợp lỗi

Không thể dự đoán mọi lỗi vendor, dataset hoặc hành vi người dùng trong tương lai. “Toàn bộ trường hợp lỗi” của baseline này nghĩa là mọi failure boundary đã biết đều có testcase và đường phục hồi. Khi phát hiện lỗi mới:

1. ghi triệu chứng, thao tác, input/fixture hash, organization/context, expected và observed;
2. gán lỗi cho phase sở hữu nguyên nhân, link FR và xác định ảnh hưởng đến các phase downstream;
3. tạo testcase hồi quy mới, không sửa expected cũ để biến lỗi thành PASS;
4. nếu thay đổi kết quả, tạo engine/model/schema/template/protocol revision tương ứng và giữ snapshot cũ;
5. chạy lại các testcase bị ảnh hưởng trên đúng candidate, cập nhật `specification.md`, `plan.md`, `implementation-progress.md` và release manifest;
