...
export function SessionErrorPage() {
  ...
  return <main className="auth-state"><section className="auth-card"><p className="eyebrow">PHIÊN ĐĂNG NHẬP</p><h1>Chưa có đơn vị</h1><div className="alert alert--error" role="alert"><h2>Tài khoản đã xác thực</h2><p>Tài khoản đã đăng nhập thành công nhưng chưa được gắn vào đơn vị RT-CONNECT nào.</p></div><section className="onboarding-section"><h2>Tạo đơn vị đầu tiên</h2><p className="auth-card__lead">Nếu đây là lần đầu thiết lập, nhập tên bệnh viện hoặc đơn vị để tạo nơi làm việc. Tài khoản hiện tại sẽ được gắn làm thành viên đầu tiên; mọi bác sĩ và kỹ sư trong cùng đơn vị vẫn thao tác ngang hàng.</p><form className="stack-form" onSubmit={submitOrganization}><label htmlFor="organization-name">Tên bệnh viện hoặc đơn vị<input id="organization-name" value={organizationName} onChange={(event) => setOrganizationName(event.target.value)} placeholder="Ví dụ: Bệnh viện Ung Bướu Thành phố" maxLength={200} autoComplete="organization" /></label>{(formError || creationError) && <div className="alert alert--error" role="alert"><p>{formError ?? creationError}</p></div>}<button type="submit" disabled={createOrganization.isPending}>{createOrganization.isPending ? 'Đang tạo đơn vị…' : 'Tạo đơn vị và mở nơi làm việc'}</button></form></section><p className="auth-card__footnote">Nếu đơn vị đã được tạo sẵn, hãy liên hệ thành viên trong đơn vị để được thêm vào.</p><button className="button-secondary" onClick={() => void signOutAndReturn()}>Đăng xuất và đăng nhập lại</button><Link className="text-link" to="/app/system/status">Trạng thái dịch vụ</Link></section></main>
}
