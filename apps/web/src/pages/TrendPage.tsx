...
export function TrendPage() {
  ...
  }), [aggregate, detector, energy, from, metricKey, phantom, selectedMachineIds, timezone, to])
  ...
  const createBaseline = useMutation({ mutationFn: () => apiClient.createTrendBaseline(accessToken!, organizationId!, { machine_id: baselineMachineId || machines[0]?.id || '', metric_key: baselineMetric, unit: baselineUnit, name: `Baseline ${baselineMetric}`, baseline_value: Number(baselineValue), tolerance: Number(baselineTolerance), effective_from: new Date().toISOString() }), onSuccess: () => { setMessage('Đã tạo baseline version mới.'); void queryClient.invalidateQueries({ queryKey: ['trend-baselines', organizationId] }); void trend.refetch() }, onError: (error) => setMessage(errorMessage(error)) })
  const createEvent = useMutation({ mutationFn: () => apiClient.createTrendEvent(accessToken!, organizationId!, { machine_id: eventMachineId || machines[0]?.id || '', event_type: 'MAINTENANCE', title: eventTitle, started_at: eventStartedAt ? new Date(eventStartedAt).toISOString() : new Date().toISOString(), notes: eventNotes }), onSuccess: () => { setMessage('Đã ghi maintenance marker.'); setEventTitle(''); setEventNotes(''); void queryClient.invalidateQueries({ queryKey: ['trend-events', organizationId] }); void trend.refetch() }, onError: (error) => setMessage(errorMessage(error)) })
  ...
  return <div className="page">
    ...
    </div><p className="form-hint">Không chọn machine nghĩa là tất cả machine trong organization. Context khác unit/energy/detector/phantom/protocol sẽ tách thành series riêng, không nối sai nghĩa.</p></section>
