...
function newTissue(index = 0): TissueDraft {
  return {
    ...
    sourceReference: 'Giả định kịch bản do người dùng nhập'
  }
}
...
function newCourse(index: number): CourseDraft {
  return {
    ...
    label: index === 0 ? 'Đợt điều trị trước' : 'Đợt điều trị hiện tại',
    isPrior: index === 0,
    ...
    recoverySourceReference: 'Giả định hồi phục do người dùng nhập',
    tissues: [newTissue()]
  }
}
...
function DownloadButton({ run, mode, format, accessToken, organizationId, onMessage }: { run: P15RunResource; mode: Mode; format: 'JSON' | 'CSV'; accessToken: string; organizationId: string; onMessage: (value: string) => void }) {
  ...
  return <button className="button-secondary" disabled={busy} onClick={() => {
    setBusy(true)
    void apiClient.downloadP15(accessToken, organizationId, mode === 'REIRRADIATION' ? 're-irradiation' : 'fraction-compensation', run.id, exportFormat).then((blob) => {
      const url = URL.createObjectURL(blob)
      const anchor = document.createElement('a')
      anchor.href = url
      anchor.download = `rt-connect-p15-${run.id}.csv`
      anchor.click()
      URL.revokeObjectURL(url)
      onMessage('Đã tải bảng kết quả.')
...
function ReIrradiationResults({ run, mode }: { run: P15RunResource; mode: Mode }) {
  ...
  if (mode === 'REIRRADIATION') {
    ...
    return <>
      <section className="p15-result-summary"><article><span>SỐ NHÓM</span><strong>{groups.length}</strong><small>Bối cảnh mô và α/β</small></article><article><span>KHÔNG HỒI PHỤC</span><strong>{formatNumber(groups[0]?.bed_no_recovery_gy)}</strong><small>BED Gy làm mốc</small></article><article><span>CÓ HỒI PHỤC</span><strong>{formatNumber(groups[0]?.bed_with_recovery_gy)}</strong><small>BED Gy theo giả định</small></article><article><span>TÍCH LŨY KHÔNG GIAN</span><strong>CHƯA CÓ</strong><small>Chỉ tính tổng liều</small></article></section>
      {warningRows.length > 0 && <div className="alert alert--warning"><strong>Giả định và cảnh báo</strong><ul>{warningRows.map((item, index) => <li key={`${String(item.code)}-${index}`}>{String(item.message ?? 'Có cảnh báo trong kết quả.')}</li>)}</ul></div>}
      <div className="p15-result-meta"><span>Kết quả đã được khóa sau khi lưu</span><span>{groups.length} nhóm mô đã tính</span></div>
      <div className="table-wrap"><table className="p15-result-table"><thead><tr><th>Mô hoặc nhóm</th><th>α/β</th><th>Tổng liều</th><th>BED không hồi phục</th><th>BED có hồi phục</th><th>EQD2 có hồi phục</th></tr></thead><tbody>{groups.map((item, index) => <tr key={`${String(item.group_key)}-${index}`}><td><strong>{String(item.tissue_key ?? '—')}</strong></td><td>{formatNumber(item.alpha_beta_gy)}</td><td>{formatNumber(item.total_dose_gy)}</td><td>{formatNumber(item.bed_no_recovery_gy)}</td><td>{formatNumber(item.bed_with_recovery_gy)}</td><td>{formatNumber(item.eqd2_with_recovery_gy)}</td></tr>)}</tbody></table></div>
      <div className="table-wrap"><table className="p15-result-table"><caption>Đóng góp theo đợt điều trị — hồi phục chỉ áp dụng một lần cho đợt trước</caption><thead><tr><th>Đợt điều trị</th><th>Đợt trước</th><th>Số nhóm mô</th><th>Hồi phục</th></tr></thead><tbody>{courses.map((item, index) => <tr key={`${String(item.course_id)}-${index}`}><td><strong>{String(item.label ?? '—')}</strong></td><td>{item.is_prior === true ? 'Có' : 'Không'}</td><td>{records(item.tissue_doses).length}</td><td>{formatNumber(item.recovery_fraction, 2)}</td></tr>)}</tbody></table></div>
      <div className="table-wrap"><table className="p15-result-table"><caption>Độ nhạy — thay đổi mức hồi phục để xem ảnh hưởng, không phải khoảng tin cậy</caption><thead><tr><th>Nhóm mô</th><th>Mức hồi phục</th><th>BED</th><th>EQD2</th></tr></thead><tbody>{sensitivity.flatMap((group, groupIndex) => records(group.points).map((point, pointIndex) => <tr key={`${groupIndex}-${pointIndex}`}><td>{String(group.group_key ?? '—')}</td><td>{formatNumber(point.recovery_fraction, 2)}</td><td>{formatNumber(point.bed_gy)}</td><td>{formatNumber(point.eqd2_gy)}</td></tr>))}</tbody></table></div>
    </>
  }
...
export function ReIrradiationPage({ mode }: { mode: Mode }) {
  ...
  const [name, setName] = useState(mode === 'REIRRADIATION' ? 'Tính toán tái xạ' : 'Tính toán bù phân liều')
  ...
  const [sourceReference, setSourceReference] = useState('Giả định do người dùng nhập')
  ...
  const [timeSource, setTimeSource] = useState('Mô hình thời gian tuyến tính do người dùng nhập')
  ...
  const buildBody = (): ReIrradiationInput | FractionCompensationInput => {
    if (!activeRevisionId) throw new Error('Cần chọn một bản cập nhật đã lưu.')
  ...
  if (bootstrap.error || !organizationId) return <div className="page"><section className="alert alert--error"><h1>Không thể mở công cụ</h1><p>{errorMessage(bootstrap.error)}</p><Link className="button-link" to="/app/biological">Quay lại công cụ sinh học</Link></section></div>
  if (scenarios.error || revisions.error) return <div className="page"><section className="alert alert--error"><h1>Không tải được dữ liệu phác thảo</h1><p>{errorMessage(scenarios.error ?? revisions.error)}</p><button onClick={() => void Promise.all([scenarios.refetch(), revisions.refetch()])}>Thử lại</button></section></div>

  return <div className="page p15-page">
    <header className="page-header"><div><p className="eyebrow">P15 · CÔNG CỤ SINH HỌC</p><h1>{mode === 'REIRRADIATION' ? 'Máy tính tái xạ' : 'Bù phân liều'}</h1><p>{mode === 'REIRRADIATION' ? 'Tính BED/EQD2 theo từng đợt điều trị và mô hoặc cơ quan, kèm giả định hồi phục.' : 'So sánh các lịch bù trên phần đã thực hiện, không thay đổi dữ liệu đã ghi nhận.'}</p></div><div className="page-header__actions"><Link className="button-link button-secondary" to="/app/biological">Quay lại công cụ sinh học</Link><span className="status-badge status-badge--warning">TÍNH TOÁN THAM KHẢO</span></div></header>
    <nav className="p15-mode-tabs" aria-label="Công cụ sinh học"><Link className={mode === 'REIRRADIATION' ? 'is-active' : ''} to="/app/biological/re-irradiation">Tái xạ</Link><Link className={mode === 'FRACTION_COMPENSATION' ? 'is-active' : ''} to="/app/biological/fraction-compensation">Bù phân liều</Link></nav>
    {message && <section className="alert alert--success" role="status"><p>{message}</p></section>}
    <section className="biological-notice p15-notice" role="note"><strong>Tính toán sinh học độc lập</strong><span>Không tạo chỉ định điều trị, không tự gắn vào hồ sơ kiểm tra, hồ sơ người bệnh, hệ thống lập kế hoạch hay lưu trữ ảnh. Phần tái xạ chỉ tính tổng liều vô hướng; giả định hồi phục, thời gian và liều cơ quan phải có nguồn rõ ràng.</span></section>
    <section className="panel p15-context-panel"><div className="panel-heading"><div><p className="eyebrow">BỐI CẢNH ĐÃ LƯU</p><h2>Chọn bối cảnh tính</h2></div><span className="status-badge">{savedRevisions.length} bản cập nhật</span></div><div className="p15-form-grid"><label>Bối cảnh đã lưu<select value={activeScenarioId} onChange={(event) => { setScenarioId(event.target.value); setRevisionId(''); setSelectedRunId(undefined) }}><option value="">Chọn bối cảnh…</option>{scenarios.data?.items.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>Bản cập nhật<select value={activeRevisionId} onChange={(event) => { setRevisionId(event.target.value); setSelectedRunId(undefined) }}><option value="">Chọn bản cập nhật…</option>{savedRevisions.map((item) => <option value={item.id} key={item.id}>Bản lần {item.revision_number} · {formatDate(item.created_at)}</option>)}</select></label><label className="p15-form-grid__wide">Tên kết quả<input value={name} onChange={(event) => setName(event.target.value)} /></label></div></section>
...
function ValidationBlock({ validation }: { validation: P15ValidationResource }) {
  return <section className={validation.valid ? 'bed-validation bed-validation--ok' : 'bed-validation bed-validation--error'}><strong>{validation.valid ? 'Thông tin hợp lệ' : 'Thông tin chưa hợp lệ'}</strong>{validation.errors.length > 0 ? <ul>{validation.errors.map((item, index) => <li key={`${item.code}-${index}`}><strong>{item.field ?? 'Thông tin nhập'}</strong>: {item.message}</li>)}</ul> : <p>{validation.warnings.length > 0 ? validation.warnings.map((item) => item.message).join(' ') : 'Đã kiểm tra xong; chưa lưu kết quả.'}</p>}</section>
}
