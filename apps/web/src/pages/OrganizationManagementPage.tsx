...
function MachineRow({
  ...
  return (
    <tr>
      ...
      <td><input disabled={machine.is_archived} aria-label={`Tên máy ${machine.stable_machine_id}`} value={displayName} onChange={(event) => setDisplayName(event.target.value)} /></td>
      <td>{machine.manufacturer ?? '—'} {machine.model ?? ''}</td>
      <td><select disabled={machine.is_archived} aria-label={`Trạng thái ${machine.stable_machine_id}`} value={status} onChange={(event) => setStatus(event.target.value)}>{machineStatuses.map((item) => <option key={item}>{item}</option>)}</select></td>
      <td className="table-actions">
        <button disabled={!dirty} onClick={() => onSave(machine, { display_name: displayName, status })}>Lưu</button>
        <button className="button-secondary" onClick={() => onToggleArchive(machine)}>{machine.is_archived ? 'Khôi phục' : 'Archive'}</button>
      </td>
    </tr>
  )
}
