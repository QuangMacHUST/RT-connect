...
function parameterChanges(
  current: Array<[string, unknown]>,
  previous: Array<[string, unknown]>,
...
export function PylinacResultPanel({ latest, history, accessToken, caseId, emptyHistoryLabel, metrics = [], resultNote, overlayLabel = 'Mở ảnh phân tích', inputArtifacts, selectedArtifactIds, renderResultDetails, structuredMetricExclusions = [], onMessage, onAssess }: PylinacResultPanelProps) {
  ...
  const previousParameters = parameterEntries(previousRun)
  const changedParameters = parameterChanges(selectedParameters, previousParameters)
...
function numericResultValue(value: unknown): string {
  return typeof value === 'number' && Number.isFinite(value)
    ? value.toLocaleString('vi-VN', { maximumFractionDigits: 3 })
    : '—'
}

export function WinstonLutzMultiTargetDetails({ run }: { run: PylinacQARunResource }): ReactNode {
  const imageDetails = records(pylinacMetric(run, 'image_details'))
  if (imageDetails.length === 0) return null

  const arrangement = records(pylinacMetric(run, 'bb_arrangement'))
  const configuredNames = arrangement
    .map((item) => typeof item.name === 'string' ? item.name.trim() : '')
    .filter(Boolean)
  const detectedNames = Object.keys(objectValue(imageDetails[0]?.bb_distances) ?? {})
...
type PylinacAdjustmentCanvasProps = {
  ...
  x: string
  y: string
  coordinateMode?: 'PIXEL' | 'NORMALIZED' | 'OFFSET_MM'
  heading?: string
  description?: string
  disabled?: boolean
...
export function PylinacAdjustmentCanvas({ accessToken, artifactId, x, y, coordinateMode = 'PIXEL', heading = 'Chọn tâm bắt đầu', description = 'Nhấn hoặc kéo trên ảnh để đặt tâm bắt đầu. Tọa độ được quy đổi theo kích thước ảnh gốc và gửi cho Pylinac.', disabled = false, controlledImageIndex, onImageIndexChange, onPointChange }: PylinacAdjustmentCanvasProps) {
  ...
  const updatePoint = (clientX: number, clientY: number) => {
    ...
    const point = mapImagePoint(clientX, clientY, bounds, image.naturalWidth, image.naturalHeight, coordinateMode, previewInfo.data?.pixel_spacing_mm ?? undefined)
  ...
  const pointStyle = coordinateMode === 'OFFSET_MM' && previewInfo.data?.pixel_spacing_mm
    ? { left: `${(imageDimensions.width / 2 + Number(x) / previewInfo.data.pixel_spacing_mm[1]) / imageDimensions.width * 100}%`, top: `${(imageDimensions.height / 2 + Number(y) / previewInfo.data.pixel_spacing_mm[0]) / imageDimensions.height * 100}%` }
...
function CatPhanPage({ caseId, accessToken, title, catalogKey }: { caseId: string; accessToken: string; title: string; catalogKey: CatPhanCatalogKey }) {
  ...
  return <div className="page">
    ...
    <section className="panel machine-qa-panel">
      ...
      {imageArtifacts.length > 0 && <label>Tệp đang chọn<select value={selectedInput ?? ''} onChange={(event) => setSelectedArtifactId(event.target.value)}>{imageArtifacts.map((artifact) => <option key={artifact.id} value={artifact.id}>{artifactDisplayName(artifact, imageArtifacts)}</option>)}</select></label>}
      {selectedInput && <PylinacAdjustmentCanvas key={`${selectedInput}-catphan-slice`} accessToken={accessToken} artifactId={selectedInput} x={xAdjustment} y={yAdjustment} coordinateMode="OFFSET_MM" heading="Chọn tâm phantom trên ảnh" description="Nhấn hoặc kéo gần tâm phantom để đặt điều chỉnh ngang–dọc theo milimét. Pylinac vẫn là bộ tính; thao tác này chỉ tạo tham số cho lượt phân tích mới." controlledImageIndex={originSlice.trim() === '' ? undefined : Number(originSlice)} onImageIndexChange={(index) => setOriginSlice(String(index))} disabled={isBusy} onPointChange={(x, y) => { setXAdjustment(x); setYAdjustment(y) }} />}
      <div className="machine-qa-protocol-controls">
...
function AcrPage({ caseId, accessToken, title, catalogKey }: { caseId: string; accessToken: string; title: string; catalogKey: AcrCatalogKey }) {
  ...
  return <div className="page">
    ...
    <section className="panel machine-qa-panel">
      ...
      <div className="machine-qa-actions"><label className="button-link">Chọn tệp ZIP DICOM<input type="file" accept=".zip,application/zip" hidden disabled={isBusy} onChange={(event) => { const file = event.target.files?.[0]; if (file) upload.mutate(file); event.currentTarget.value = '' }} /></label></div>
      {imageArtifacts.length > 0 && <label>Tệp đang chọn<select value={selectedInput ?? ''} onChange={(event) => setSelectedArtifactId(event.target.value)}>{imageArtifacts.map((artifact) => <option key={artifact.id} value={artifact.id}>{artifactDisplayName(artifact, imageArtifacts)}</option>)}</select></label>}
      {selectedInput && <PylinacAdjustmentCanvas key={`${selectedInput}-acr-slice`} accessToken={accessToken} artifactId={selectedInput} x={xAdjustment} y={yAdjustment} coordinateMode="OFFSET_MM" heading="Chọn tâm phantom trên ảnh" description="Nhấn hoặc kéo gần tâm phantom để đặt điều chỉnh ngang–dọc theo milimét. Lựa chọn này chỉ áp dụng cho lượt phân tích mới." controlledImageIndex={originSlice.trim() === '' ? undefined : Number(originSlice)} onImageIndexChange={(index) => setOriginSlice(String(index))} disabled={isBusy} onPointChange={(x, y) => { setXAdjustment(x); setYAdjustment(y) }} />}
      <div className="machine-qa-protocol-controls">
        <label>Lát gốc tùy chọn<input type="number" min="0" step="1" placeholder="Tự động" value={originSlice} onChange={(event) => setOriginSlice(event.target.value)} /></label>
        <label>Điều chỉnh ngang (mm)<input type="number" step="0.1" value={xAdjustment} onChange={(event) => setXAdjustment(event.target.value)} /></label>
        <label>Điều chỉnh dọc (mm)<input type="number" step="0.1" value={yAdjustment} onChange={(event) => setYAdjustment(event.target.value)} /></label>
...
function CtPylinacPage({ caseId, accessToken, title, catalogKey }: { caseId: string; accessToken: string; title: string; catalogKey: CtPylinacCatalogKey }) {
  ...
  return <div className="page">
    ...
    <section className="panel machine-qa-panel">
      ...
      <div className="machine-qa-actions"><label className="button-link">Chọn tệp ZIP DICOM<input type="file" accept=".zip,application/zip" hidden disabled={isBusy} onChange={(event) => { const file = event.target.files?.[0]; if (file) upload.mutate(file); event.currentTarget.value = '' }} /></label></div>
      {imageArtifacts.length > 0 && <label>Tệp đang chọn<select value={selectedInput ?? ''} onChange={(event) => setSelectedArtifactId(event.target.value)}>{imageArtifacts.map((artifact) => <option key={artifact.id} value={artifact.id}>{artifactDisplayName(artifact, imageArtifacts)}</option>)}</select></label>}
      {selectedInput && <PylinacAdjustmentCanvas key={`${selectedInput}-ct-slice`} accessToken={accessToken} artifactId={selectedInput} x={xAdjustment} y={yAdjustment} coordinateMode="OFFSET_MM" heading="Chọn tâm phantom trên ảnh" description="Nhấn hoặc kéo gần tâm phantom để đặt điều chỉnh ngang–dọc theo milimét. Lựa chọn này chỉ áp dụng cho lượt phân tích mới." controlledImageIndex={originSlice.trim() === '' ? undefined : Number(originSlice)} onImageIndexChange={(index) => setOriginSlice(String(index))} disabled={isBusy} onPointChange={(x, y) => { setXAdjustment(x); setYAdjustment(y) }} />}
      <div className="machine-qa-protocol-controls">
        <label>Lát gốc tùy chọn<input type="number" min="0" step="1" placeholder="Tự động" value={originSlice} onChange={(event) => setOriginSlice(event.target.value)} /></label>
        <label>Điều chỉnh ngang (mm)<input type="number" step="0.1" value={xAdjustment} onChange={(event) => setXAdjustment(event.target.value)} /></label>
        <label>Điều chỉnh dọc (mm)<input type="number" step="0.1" value={yAdjustment} onChange={(event) => setYAdjustment(event.target.value)} /></label>
...
function LogAnalyzerPage({ caseId, accessToken, title, catalogKey }: { caseId: string; accessToken: string; title: string; catalogKey: LogCatalogKey }) {
  ...
  const selectionValidationError = validateLogArtifactSelection(catalogKey, selected)
  const canAnalyze = !selectionValidationError && artifactsAreValidated(selected)
  const analyze = useMutation({
  ...
  const gammaValidationError = validateLogGammaValues(calcGamma, doseTolerance, distanceTolerance)
  return <div className="page">
...
function ContribPage({ caseId, accessToken, title, catalogKey }: { caseId: string; accessToken: string; title: string; catalogKey: ContribCatalogKey }) {
  ...
  const metrics = Object.entries(objectValue(latest?.result_snapshot.metrics) ?? {}).filter(([, value]) => typeof value === 'number' || typeof value === 'string')
  const isBusy = upload.isPending || analyze.isPending || assess.isPending
  const validationError = validateContribValues({ catalogKey, fwxm, bbEdgeThreshold })
  const canAnalyze = Boolean(selectedArtifactId) && selectedArtifactsAreValidated(selectedArtifactId ? [selectedArtifactId] : [], imageArtifacts) && !isBusy && !validationError
