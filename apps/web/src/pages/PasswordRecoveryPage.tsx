...
export function PasswordRecoveryPage() {
  ...
  return <main className="auth-state"><section className="auth-card"><p className="eyebrow">ACCOUNT RECOVERY</p><h1>Khôi phục truy cập</h1><p>Nhập email tổ chức để nhận liên kết an toàn.</p>{message && <div className="alert" role="status">{message}</div>}<form onSubmit={submit}><label htmlFor="recovery-email">Email</label><input id="recovery-email" onChange={(event) => setEmail(event.target.value)} required type="email" value={email} /><button disabled={!configured || busy} type="submit">{busy ? 'Đang gửi…' : 'Gửi liên kết khôi phục'}</button></form><Link className="text-link" to="/auth/login">Quay lại đăng nhập</Link></section></main>
}
